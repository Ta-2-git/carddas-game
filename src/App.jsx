...
// 例: “/models/goku.glb”
// アニメーションクリップ名: “idle”, “attack”, “hit”, “win”, “lose”, “dash”, “kick”, “beam”
const MODEL_PATHS = {
“c001”: “”,  // 孫悟空: “/models/goku.glb”
“c002”: “”,  // ブルーS太郎
“c003”: “”,  // グレートS太郎
“c004”: “”,  // ダークS太郎
“c005”: “”,  // ファイヤーS太郎
...
// ===== 3Dキャラクターコンポーネント =====
// GLBファイルがある場合は3D表示、なければ2D(StickmanFighter)にフォールバック
const CharacterModel3D = ({ modelPath, animState = “idle”, isEnemy = false, size = 110, card, fallback2D }) => {
...
camera.position.set(isEnemy ? -1.5 : 1.5, 1.2, 4);
...
// キャラカラーのリムライト
if (card?.color) {
  const rimLight = new THREE.PointLight(card.color, 0.6, 10);
  rimLight.position.set(isEnemy ? 2 : -2, 1, -1);
  scene.add(rimLight);
}
...
const loadModel = async () => {
  try {
    // three.js r128 では GLTFLoader を別途読み込む必要あり
      ...
      (gltf) => {
        ...
        if (gltf.animations.length > 0) {
          ...
          gltf.animations.forEach(clip => {
            animActionsRef.current[clip.name.toLowerCase()] = mixer.clipAction(clip);
          });
          // 最初のアニメーション再生
          playAnim("idle");
        }
...
}, [modelPath, isEnemy, size]);

// アニメーション切り替え関数
const playAnim = useCallback((name) => {
const actions = animActionsRef.current;
const normName = name.toLowerCase();
// 対応するクリップを探す（完全一致 → 部分一致）
const key = Object.keys(actions).find(k => k === normName)
|| Object.keys(actions).find(k => k.includes(normName))
...
// animState 変化でアニメーション切り替え
useEffect(() => {
if (!modelLoadedRef.current) return;
// animState → クリップ名マッピング
const stateMap = {
“idle”: “idle”, “kaioken_idle”: “idle”,
“attack”: “attack”, “punch”: “attack”, “kick”: “kick”, “beam”: “beam”,
“dash”: “dash”, “hit”: “hit”, “win”: “win”, “lose”: “lose”,
};
playAnim(stateMap[animState] || “idle”);
}, [animState, playAnim]);

// GLBがない場合は2Dフォールバック
if (!modelPath) {
return fallback2D || null;
}
...
filter: animState === “kaioken_idle”
? “drop-shadow(0 0 12px #ff4400) drop-shadow(0 0 24px #ff220088)”
: animState === “hit” ? “brightness(2) drop-shadow(0 0 8px #fff)”
: “none”,
...
// ===== キャラクター表示ラッパー（3D/2D自動切り替え） =====
// isGoku → GokuFighter(2D) → CharacterModel3D(3D) の優先順で
const CharacterFighter = ({ card, animState, isEnemy = false, size = 110, scale = 1 }) => {
const modelPath = MODEL_PATHS[card.id] || “”;
const fallback = card.isGoku
? <GokuFighter animState={animState} isEnemy={isEnemy} scale={scale} />
: <StickmanFighter card={card} isEnemy={isEnemy} size={size} state={animState} />;
...
color: “#ef4444”,
glow: “#dc2626”,
description: “1段階変身した状態でバトル開始。孫悟空は界王拳状態からスタート。”,
timing: “battle_start”,
effect: “transform_start”,
illustSymbol: “🔥”,
},
{
id: “s002”,
...
color: “#a78bfa”,
glow: “#7c3aed”,
description: “HPが300、ATKが100上昇する。（永続）”,
timing: “battle_start”,
effect: “boost_stats”,
hpBoost: 300,
atkBoost: 100,
...
color: “#22d3ee”,
glow: “#0891b2”,
description: “5ターン以内に自分の体力が0になった時、体力を半分回復する。(1回限り)”,
timing: “on_ko”,         // KO時に発動
condition: “turn_lte_5”, // 5ターン以内
effect: “revive_half”,   // 効果種別
illustSymbol: “💧”,
},
];
...
function calcDamage(atk, moveId) {
const move = MOVES[moveId];
return Math.max(1, Math.floor(atk * move.power * (0.95 + Math.random() * 0.1)));
}
...
if (animState === ‘idle’) return <div style={baseStyle}><IdleAnim frames={IDLE_FRAMES} fps={8} scale={scale} flipH={flip} /></div>;
if (animState === ‘kaioken_idle’) return <div style={{ …baseStyle, filter: ‘drop-shadow(0 0 16px #ff3300) drop-shadow(0 0 32px #ff110066)’ }}><IdleAnim frames={KAIOKEN_IDLE_FRAMES} fps={8} scale={scale} flipH={flip} pingPong={true} /></div>;
if (animState === ‘dash’) return <div style={baseStyle}><IdleAnim frames={DASH_FRAMES} fps={12} scale={scale} flipH={flip} /></div>;
if (animState === ‘punch’) return <div style={baseStyle}><IdleAnimOnce frames={PUNCH_FRAMES} fps={12} scale={scale} flipH={flip} /></div>;
if (animState === ‘kick’) return <div style={baseStyle}><IdleAnimOnce frames={KICK_FRAMES} fps={12} scale={scale} flipH={flip} /></div>;
if (animState === ‘beam’) return <div style={{ …baseStyle, filter: ‘drop-shadow(0 0 20px #0088ff) drop-shadow(0 0 40px #0044ff88)’ }}><SpriteOnce sheet={IMG_BEAM_SHEET} frameW={BEAM_FRAME.w} frameH={BEAM_FRAME.h} frameCount={BEAM_FRAME.count} fps={12} scale={scale} flipH={flip} /></div>;
if (animState === ‘hit’) return <div style={{ …baseStyle, filter: ‘brightness(3) drop-shadow(0 0 8px white)’ }}><SpriteAnim sheet={IMG_PUNCH_SHEET} frameW={PUNCH_FRAME.w} frameH={PUNCH_FRAME.h} frameCount={PUNCH_FRAME.count} fps={3} frameIndex={2} scale={scale} flipH={flip} /></div>;
if (animState === ‘win’) return <div style={{ …baseStyle, animation: ‘winBounce 0.5s infinite alternate’, filter: ‘drop-shadow(0 0 16px #ffd700)’ }}><SpriteAnim sheet={IMG_PUNCH_SHEET} frameW={PUNCH_FRAME.w} frameH={PUNCH_FRAME.h} frameCount={PUNCH_FRAME.count} fps={2} frameIndex={0} scale={scale} flipH={flip} /></div>;
if (animState === ‘lose’) return <div style={{ …baseStyle, opacity: 0.5, transform: ‘rotate(20deg) translateY(10px)’, filter: ‘grayscale(1)’ }}><SpriteAnim sheet={IMG_PUNCH_SHEET} frameW={PUNCH_FRAME.w} frameH={PUNCH_FRAME.h} frameCount={PUNCH_FRAME.count} fps={1} frameIndex={4} scale={scale} flipH={flip} /></div>;
return null;
};
...
<line key={`d1-${i}`} x1={i * 14 - h} y1={0} x2={i * 14} y2={h} stroke=”#f59e0b” strokeWidth=“0.5” />
))}
{Array.from({ length: 20 }).map((*, i) => (
<line key={`d2-${i}`} x1={i * 14} y1={0} x2={i * 14 - h} y2={h} stroke=”#f59e0b” strokeWidth=“0.5” />
))}
</svg>
...
<text key={i} x={hpNumX} y={VH * 0.2} textAnchor={hpNumAnchor} dominantBaseline=“middle” fontFamily=”‘Arial Black’,‘Impact’,sans-serif” fontWeight=“900” fontSize=“57” fill=”#000” opacity=“0.95” transform={`translate(${dx},${dy})`}>{Math.max(0, Math.round(displayHp))}</text>
))}
...
{[[-2, -2], [2, -2], [-2, 2], [2, 2]].map(([dx, dy], i) => (
<text key={i} x={labelX} y={VH * 0.2} textAnchor={labelAnchor} dominantBaseline=“middle” fontFamily=”‘Arial Black’,‘Impact’,sans-serif” fontWeight=“900” fontSize=“28” fill=”#000” opacity=“0.95” transform={`translate(${dx},${dy})`}>{playerLabel}</text>
))}
...
// ランダムにN個のスロットを配置するヘルパー
// clusterStrength: 0.0〜1.0 で「まとまりやすさ」を制御
//   0.0 = 完全ランダム散布
//   0.5 = ゆるくまとまる
//   1.0 = しっかりまとまる（それでも10%でのみ完全連番）
...
// clusterStrength に応じて「グループサイズ」と「グループ数」を決定
// clusterStrength=0 → 1個ずつバラバラ
// clusterStrength=0.5 → 2個グループ
// clusterStrength=1.0 → 最大グループ（ただし完全連番ではない）
const maxGroupSize = Math.max(1, Math.round(1 + clusterStrength * (count - 1)));
let placed = 0;

while (placed < count) {
// グループサイズをランダムに決定（1〜maxGroupSize）
...
// グループを配置（完全連番ではなく、隣接しつつ少しばらつかせる）
let cur = start;
for (let k = 0; k < groupSize && placed < count; k++) {
  slots[cur % total] = type;
  occupied.add(cur % total);
  placed++;
  // clusterStrength < 1.0 の場合、時々1〜2マススキップ
  const skip = (clusterStrength < 1.0 && Math.random() > clusterStrength)
...
if (lv === 1) {
// 2マス: ほぼバラバラ
placeSlots(ROULETTE_TOTAL, 2, “special”, slots, 0.0);
} else if (lv === 2) {
// 3マス: ゆるくまとまる
placeSlots(ROULETTE_TOTAL, 3, “special”, slots, 0.5);
} else {
// 4マス: ある程度まとまる（完全連番は10%のみ）
placeSlots(ROULETTE_TOTAL, 4, “special”, slots, 0.8);
}
return slots;
}
...
// ===== GACHA RESULT SCREEN (カード裏→表フリップ) =====
// mode: “gacha” = 購入モード（もう一度購入 / ホームへ）
// mode: “game”  = ゲームモード（カードを選択する のみ）
const GachaResultScreen = ({ card, mode = “gacha”, onHome, onBuyAgain, onSelectCard }) => {
const [phase, setPhase] = useState(0);
// phase 0: nothing, 1: card back appears, 2: flip to front, 3: show details, 4: show buttons
const rar = RARITY_CONFIG[card.rarity];
useEffect(() => {
...
```
  <div style={{ position: "relative", zIndex: 1, textAlign: "center" }}>
    {phase >= 1 && (
      <div style={{ fontSize: 12, color: rar.color, letterSpacing: 4, marginBottom: 20, animation: "fadeInUp 0.5s ease", textShadow: `0 0 20px ${rar.color}` }}>✦ カード排出！ ✦</div>
    )}
    ...
    {phase >= 1 && (
      <div style={{ perspective: "1000px", marginBottom: 20 }}>
        <div style={{
          width: 130, height: 182, position: "relative",
          transformStyle: "preserve-3d",
          transition: phase >= 2 ? "transform 0.9s cubic-bezier(0.4,0,0.2,1)" : "none",
          transform: phase >= 2 ? "rotateY(180deg)" : "rotateY(0deg)",
          margin: "0 auto",
          animation: phase === 1 ? "cardAppear 0.5s cubic-bezier(0.34,1.56,0.64,1)" : "none",
        }}>
          {/* Back face */}
          ...
          <div style={{ position: "absolute", inset: 0, backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", transform: "rotateY(180deg)" }}>
            <div style={{ width: 130, height: 182, borderRadius: 10, background: card.isGoku ? "linear-gradient(160deg,#1a0500,#2d1000,#1a0800)" : "linear-gradient(160deg,#1a1a2e,#16213e,#0f3460)", border: `3px solid ${rar.color}`, boxShadow: `0 0 40px ${rar.glow}88`, overflow: "hidden", display: "flex", flexDirection: "column", alignItems: "center", padding: "6px 4px 4px" }}>
              ...
              <div style={{ marginTop: 14, flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
                {card.isGoku && IMG_CARD_GOKU ? <img src={IMG_CARD_GOKU} alt={card.name} style={{ height: 100, width: "auto" }} /> : <StickmanFighter card={card} size={72} state="win" />}
              </div>
    ...
    {phase >= 3 && (
      <div style={{ animation: "fadeInUp 0.5s ease", background: "rgba(0,0,0,0.7)", border: `1px solid ${rar.color}44`, borderRadius: 12, padding: "14px 20px", maxWidth: 260, margin: "0 auto 16px" }}>
        ...
        <div style={{ background: "#0f172a", borderRadius: 8, padding: "8px 10px", border: "1px solid #ef444433" }}>
          ...
          {[card.rock, card.scissors, card.paper]
            .filter((v, i, a) => a.indexOf(v) === i)
            .filter(moveId => MOVES[moveId].type === "special")
            .map((moveId, i) => {
              const m = MOVES[moveId];
              return (
                <div key={i} style={{ fontSize: 13, color: m.color, fontWeight: "700", textShadow: `0 0 8px ${m.color}`, letterSpacing: 1 }}>
                  💥 {m.name}
                </div>
              );
            })
    ...
    {phase >= 4 && (
      <div style={{ animation: "fadeInUp 0.4s ease", display: "flex", flexDirection: "column", gap: 10, alignItems: "center" }}>
        {mode === "game" ? (
          // ゲームモード：カードを選択するのみ
          <button onClick={onSelectCard} style={{ padding: "16px 48px", background: "linear-gradient(135deg,#f59e0b,#d97706)", border: "none", borderRadius: 8, color: "#000", fontSize: 15, fontWeight: "900", cursor: "pointer", fontFamily: "monospace", boxShadow: "0 4px 24px #f59e0b99", letterSpacing: 1 }}>
            🃏 カードを選択する
          </button>
        ) : (
          // 購入モード：もう一度購入 / ホームへ
          <>
            <button onClick={onBuyAgain} style={{ padding: "14px 40px", background: "linear-gradient(135deg,#f59e0b,#d97706)", border: "none", borderRadius: 8, color: "#000", fontSize: 14, fontWeight: "900", cursor: "pointer", fontFamily: "monospace", boxShadow: "0 4px 20px #f59e0b88", letterSpacing: 1 }}>
              🎴 もう一度購入する
            </button>
            <button onClick={onHome} style={{ padding: "12px 40px", background: "transparent", border: "2px solid #6b7280", borderRadius: 8, color: "#9ca3af", fontSize: 13, fontWeight: "700", cursor: "pointer", fontFamily: "monospace" }}>
              🏠 ホームへ
            </button>
          </>
        )}
...
{Array.from({ length: rows + 1 }).map((*, i) => (
<line key={`h${i}`} x1=“0” y1={`${(i / rows) * 100}%`} x2=“100%” y2={`${(i / rows) * 100}%`} stroke=”#00ff4433” strokeWidth=“0.5” />
))}
{Array.from({ length: cols + 1 }).map((*, i) => (
<line key={`v${i}`} x1={`${(i / cols) * 100}%`} y1=“0” x2={`${(i / cols) * 100}%`} y2=“100%” stroke=”#00ff4433” strokeWidth=“0.5” />
))}
</svg>
{/* Scanline */}
...
<div key={i} style={{ position: “absolute”, top: ry === 0 ? 8 : undefined, bottom: ry === 1 ? 8 : undefined, left: rx === 0 ? 8 : undefined, right: rx === 1 ? 8 : undefined, width: 20, height: 20, borderTop: ry === 0 ? “2px solid #00ff44” : “none”, borderBottom: ry === 1 ? “2px solid #00ff44” : “none”, borderLeft: rx === 0 ? “2px solid #00ff44” : “none”, borderRight: rx === 1 ? “2px solid #00ff44” : “none” }} />
))}
{/* Floating hex data */}
{Array.from({ length: 8 }).map((_, i) => (
<div key={i} style={{ position: “absolute”, left: `${10 + i * 12}%`, top: `${15 + (i % 3) * 25}%`, fontSize: 8, color: “#00ff4422”, fontFamily: “monospace”, animation: `dataFloat ${2 + i * 0.3}s ease-in-out infinite alternate`, userSelect: “none” }}>
...
// ===== DATA ASSEMBLE EFFECT (デジタル出現アニメ) =====
...
{/* Assemble overlay */}
<div style={{ position: “relative”, filter: scanLines < 20 ? `brightness(${0.3 + scanLines * 0.035}) saturate(${scanLines * 0.05})` : “none”, transition: “filter 0.1s”, animation: scanLines >= 20 ? “none” : undefined }}>
{/* Scan bars effect */}
{scanLines < 20 && Array.from({ length: 10 }).map((_, i) => (
<div key={i} style={{ position: “absolute”, left: 0, right: 0, height: `${100 / 10}%`, top: `${i * 10}%`, background: i * 2 < scanLines ? “transparent” : “rgba(0,255,68,0.15)”, pointerEvents: “none”, zIndex: 5, transition: “background 0.1s” }} />
))}
{card.isGoku
? <GokuFighter animState="idle" scale={0.85} />
: <StickmanFighter card={card} size={110} state="idle" />
}
</div>
{/* Data readout */}
...
// ===== カードカットイン演出 =====
// バトル画面内オーバーレイとして表示（position:fixed でバトル画面を覆う）
const CardCutIn = ({ card, cardType, onDone }) => {
const [phase, setPhase] = useState(“cutin”); // cutin → card → done
useEffect(() => {
...
return (
<div style={{
position: “fixed”, inset: 0, zIndex: 95, overflow: “hidden”,
opacity: phase === “fadeout” ? 0 : 1,
transition: phase === “fadeout” ? “opacity 0.6s ease” : “none”,
background: isSupport
? “linear-gradient(160deg,rgba(26,0,0,0.94),rgba(48,0,16,0.96),rgba(26,5,0,0.94))”
: “linear-gradient(160deg,rgba(0,16,26,0.94),rgba(0,26,48,0.96),rgba(0,10,16,0.94))”,
}}>
{/* 放射線 */}
{Array.from({ length: 12 }).map((_, i) => (
<div key={i} style={{ position: “absolute”, top: “50%”, left: “50%”, width: “200vw”, height: 1.5, background: `linear-gradient(90deg,transparent 30%,${ac}44,transparent 70%)`, transform: `rotate(${i * 30}deg)`, transformOrigin: “0 50%”, animation: “rayRotate 5s linear infinite” }} />
))}
...
```
  ...
  {phase === "cutin" && (
    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", animation: "cutInBannerIn 0.4s cubic-bezier(0.22,1,0.36,1) forwards" }}>
      <div style={{ textAlign: "center" }}>
        <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 28, color: "#fff", textShadow: `0 0 30px ${ac}, 0 0 60px ${ac}88, 3px 3px 0 #000`, letterSpacing: 3, lineHeight: 1.3, WebkitTextStroke: `1px ${ac}` }}>
          {isSupport ? "⚡ サポートカード" : "🌟 イベントカード"}
        </div>
        <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 36, color: ac, textShadow: `0 0 20px ${ac}, 0 0 40px ${ac}88`, letterSpacing: 4, marginTop: 4 }}>
          発動！
        </div>
      </div>
    </div>
  ...
  {(phase === "card" || phase === "effect" || phase === "fadeout") && (
    <div style={{
      ...
      <div style={{ animation: "fadeInUp 0.3s ease", textAlign: "center" }}>
        <div style={{ display: "inline-block", background: `linear-gradient(135deg,${ac}22,${ac}44)`, border: `2px solid ${ac}`, borderRadius: 8, padding: "5px 18px", fontSize: 12, fontWeight: "900", color: ac, fontFamily: "'Courier New',monospace", letterSpacing: 3, textShadow: `0 0 12px ${ac}` }}>
          {isSupport ? "⚡ サポートカード発動！" : "🌟 イベントカード発動！"}
        </div>
      </div>
      ...
      <div style={{ animation: phase === "card" ? "cutInCardIn 0.5s cubic-bezier(0.34,1.56,0.64,1) both" : "none" }}>
        <div style={{
          width: 130, height: 182, borderRadius: 10,
          background: `linear-gradient(160deg,${ac}18,${ac}08,${isSupport ? "#0a0010" : "#000a10"})`,
          border: `2px solid ${ac}`,
          boxShadow: `0 0 30px ${ac}aa, 0 0 60px ${ac}44`,
          position: "relative", overflow: "hidden",
          display: "flex", flexDirection: "column", alignItems: "center", padding: "6px 4px 6px",
          userSelect: "none",
        }}>
          ...
          {/* 光沢 */}
          <div style={{ position: "absolute", top: 0, left: "-50%", width: "30%", height: "100%", background: "linear-gradient(90deg,transparent,rgba(255,255,255,0.08),transparent)", transform: "skewX(-15deg)", pointerEvents: "none" }} />
        </div>
      </div>
  ...
  {phase === "effect" && (
    <div style={{ position: "absolute", bottom: "8%", left: 0, right: 0, textAlign: "center", animation: "fadeInUp 0.3s ease" }}>
      <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 20, color: "#fff", textShadow: `0 0 20px ${ac},2px 2px 0 #000`, letterSpacing: 4, animation: "pulse 0.5s infinite" }}>✦ 効果付与！ ✦</div>
    </div>
  )}
</div>
```
...
{cards.length === 0 ? (
<div style={{ textAlign: “center”, color: “#6b7280”, fontSize: 12, padding: 40 }}>サポートカードがありません</div>
) : (
...
<div style={{
width: 130, height: 182, borderRadius: 10,
background: `linear-gradient(160deg,${card.color}18,${card.color}08,#0a0010)`,
border: `2px solid ${selected?.id === card.id ? card.color : card.color + "55"}`,
boxShadow: selected?.id === card.id ? `0 0 20px ${card.color}, 0 0 40px ${card.color}66` : `0 0 10px ${card.color}33`,
cursor: “pointer”, position: “relative”, overflow: “hidden”,
transition: “all 0.2s”, transform: selected?.id === card.id ? “scale(1.06) translateY(-4px)” : “scale(1)”,
display: “flex”, flexDirection: “column”, alignItems: “center”, padding: “6px 4px 6px”, userSelect: “none”,
}}>
{/* レアリティバー */}
...
{/* 選択チェック */}
{selected?.id === card.id && (
<div style={{ position: “absolute”, top: 4, right: 4, width: 18, height: 18, borderRadius: “50%”, background: card.color, display: “flex”, alignItems: “center”, justifyContent: “center”, fontSize: 11, fontWeight: “900”, color: “#000” }}>✓</div>
)}
{/* 光沢 */}
<div style={{ position: “absolute”, top: 0, left: “-50%”, width: “30%”, height: “100%”, background: “linear-gradient(90deg,transparent,rgba(255,255,255,0.06),transparent)”, transform: “skewX(-15deg)”, pointerEvents: “none” }} />
</div>
</div>
))}
</div>
...
{selected && (
<button onClick={() => onSelect(selected)} style={{ padding: “14px”, background: `linear-gradient(135deg,${selected.color},${selected.glow})`, border: “none”, borderRadius: 8, color: “#000”, fontSize: 14, fontWeight: “900”, cursor: “pointer”, fontFamily: “monospace”, boxShadow: `0 4px 20px ${selected.color}88` }}>
✦ このカードを使う
</button>
)}
<button onClick={() => onSkip(null)} style={{ padding: “12px”, background: “transparent”, border: “1px solid #374151”, borderRadius: 8, color: “#6b7280”, fontSize: 12, cursor: “pointer”, fontFamily: “monospace” }}>
使わずに進む →
</button>
</div>
</div>
...
<div style={{ flex: 1, overflowY: “auto”, padding: “0 16px” }}>
{cards.length === 0 ? (
<div style={{ textAlign: “center”, color: “#6b7280”, fontSize: 12, padding: 40 }}>イベントカードがありません</div>
) : (
<div style={{ display: “grid”, gridTemplateColumns: “repeat(auto-fill,minmax(130px,1fr))”, gap: 12, justifyItems: “center”, paddingBottom: 100 }}>
{cards.map(card => (
<div key={card.id} onClick={() => setSelected(selected?.id === card.id ? null : card)} style={{ cursor: “pointer” }}>
...
<div style={{
width: 130, height: 182, borderRadius: 10,
background: `linear-gradient(160deg,${card.color}18,${card.color}08,#000a10)`,
border: `2px solid ${selected?.id === card.id ? card.color : card.color + "55"}`,
boxShadow: selected?.id === card.id ? `0 0 20px ${card.color}, 0 0 40px ${card.color}66` : `0 0 10px ${card.color}33`,
cursor: “pointer”, position: “relative”, overflow: “hidden”,
transition: “all 0.2s”, transform: selected?.id === card.id ? “scale(1.06) translateY(-4px)” : “scale(1)”,
...
<div style={{ fontSize: 11, color: “#fff”, fontWeight: “700”, textAlign: “center”, padding: “2px 4px 0”, fontFamily: “‘Courier New’,monospace”, textShadow: `0 0 8px ${card.color}`, lineHeight: 1.2 }}>{card.name}</div>
{/* 発動条件 */}
<div style={{ fontSize: 7, color: card.color + “bb”, textAlign: “center”, padding: “2px 4px 0”, lineHeight: 1.3 }}>
{card.condition === “turn_lte_5” ? “5T以内KO時” : card.condition}
</div>
{/* 効果テキスト */}
...
const [selected, setSelected] = useState(null);
const [fuseCandidate, setFuseCandidate] = useState(null); // フュージョン相手候補
...
// フュージョン可能判定
const canFuse = selected && fuseCandidate
&& selected.canFuse && selected.fuseWith === fuseCandidate.id;

const handleSelect = (card) => {
if (selected?.id === card.id) { setSelected(null); setFuseCandidate(null); setAssembled(false); return; }
// フュージョン候補チェック
if (selected && selected.canFuse && selected.fuseWith === card.id) {
setFuseCandidate(card);
setAssembled(false);
setTimeout(() => setAssembled(true), 50);
return;
}
if (selected && card.canFuse && card.fuseWith === selected.id) {
setFuseCandidate(selected);
setSelected(card);
setAssembled(false);
setTimeout(() => setAssembled(true), 50);
return;
...
const [fusedCard, setFusedCard] = useState(null); // フュージョン完了後カード

const handleFuse = () => {
setShowFuseAnim(true);
setTimeout(() => {
setShowFuseAnim(false);
// フュージョン完了 → ゴテンクスカードを選択状態でデータスキャン画面へ
setFusedCard(GOTENKS_CARD);
setSelected(GOTENKS_CARD);
setFuseCandidate(null);
setAssembled(false);
...
const displayCard = canFuse ? null : selected;
...
```
  ...
  {showFuseAnim && (
    <div style={{ position: "fixed", inset: 0, zIndex: 99, background: "#000", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 0 }}>
        <div style={{ animation: "fusionCharLeft 1.5s ease forwards" }}>
          <StickmanFighter card={selected} size={80} state="attack" />
        </div>
        <div style={{ fontSize: 32, color: "#fbbf24", animation: "pulse 0.2s infinite", margin: "0 8px" }}>✦</div>
        <div style={{ animation: "fusionCharRight 1.5s ease forwards" }}>
          <StickmanFighter card={fuseCandidate} size={80} state="attack" />
        </div>
      </div>
  ...
  {/* Card grid — フュージョン後はゴテンクスのみ表示 */}
  <div style={{ flex: 1, overflowY: "auto", padding: "0 12px" }}>
    {fusedCard ? (
      // フュージョン完了：ゴテンクスカード1枚だけ中央に表示
        ...
        {cards.map(card => {
          ...
          return (
            <div key={card.id} onClick={() => handleSelect(card)} style={{ cursor: "pointer", position: "relative" }}>
              ...
              {isFuseCandidate && (
                <div style={{ position: "absolute", top: -6, left: "50%", transform: "translateX(-50%)", background: "#a78bfa", color: "#000", fontSize: 8, fontWeight: "900", padding: "1px 6px", borderRadius: 8, whiteSpace: "nowrap", boxShadow: "0 0 8px #a78bfa" }}>FUSION PAIR</div>
              )}
              {card.canFuse && !isSelected && !isFuseCandidate && (
                <div style={{ position: "absolute", bottom: 4, left: "50%", transform: "translateX(-50%)", background: "#a78bfa33", border: "1px solid #a78bfa66", color: "#a78bfa", fontSize: 7, fontWeight: "700", padding: "1px 5px", borderRadius: 6, whiteSpace: "nowrap" }}>⚡ FUSION</div>
              )}
            </div>
          );
        })}
  ...
  <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "linear-gradient(180deg,rgba(0,10,0,0.95) 0%,rgba(0,20,0,0.98) 100%)", borderTop: `2px solid ${canFuse ? "#a78bfa44" : "#00ff4444"}`, minHeight: selected ? 260 : 60, transition: "min-height 0.4s ease", overflow: "hidden" }}>
    <DigitalMatrixBg />
    {!selected && (
      <div style={{ padding: "16px", textAlign: "center", color: "#00ff4466", fontSize: 10, letterSpacing: 3, position: "relative", zIndex: 1 }}>
        ▲ カードを選んでください ▲
      </div>
    )}
    ...
    {canFuse && (
      <div style={{ padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, position: "relative", zIndex: 1 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
          <button onClick={handleFuse} style={{ padding: "14px 16px", background: "linear-gradient(135deg,#7c3aed,#a78bfa)", border: "none", borderRadius: 8, color: "#fff", fontSize: 13, fontWeight: "900", cursor: "pointer", fontFamily: "monospace", boxShadow: "0 4px 20px #a78bfa88", letterSpacing: 1, animation: "pulse 1s infinite" }}>
            ⚡ フュージョン！
          </button>
          <button onClick={() => onSelect(selected)} style={{ padding: "10px 16px", background: "linear-gradient(135deg,#00ff44,#00cc33)", border: "none", borderRadius: 8, color: "#000", fontSize: 11, fontWeight: "900", cursor: "pointer", fontFamily: "monospace" }}>
            ▶ このままバトルへ
          </button>
    ...
    {selected && !canFuse && (
      <div style={{ padding: "12px 16px", display: "flex", alignItems: "flex-end", gap: 20, position: "relative", zIndex: 1 }}>
        <div style={{ flex: 1, display: "flex", justifyContent: "center" }}>
          <DataAssembleChar card={selected} assembled={assembled} />
        </div>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
          {selected.canFuse && (
            <div style={{ fontSize: 9, color: "#a78bfa", fontFamily: "monospace", border: "1px solid #a78bfa44", borderRadius: 6, padding: "4px 8px", background: "#a78bfa11" }}>
              ⚡ {CARDS.find(c => c.id === selected.fuseWith)?.name} を選ぶとフュージョン！
            </div>
          )}
    ...
    {!selected && (
      <div style={{ textAlign: "center", padding: "0 16px 12px", position: "relative", zIndex: 1 }}>
        <button onClick={onBack} style={{ padding: "10px 30px", background: "transparent", border: "1px solid #00ff4444", borderRadius: 8, color: "#00ff4488", fontFamily: "monospace", cursor: "pointer", fontSize: 11 }}>← もどる</button>
      </div>
    )}
  </div>
</div>
...
// ===== DRAGON BURST 200% DECIDE (improved ATK comparison) =====
...
<div style={{ position: “absolute”, inset: 0, background: “linear-gradient(135deg,#2d0000 0%,transparent 40%,transparent 60%,#001520 100%)” }} />
...
```
  ...
  <div style={{ position: "absolute", top: "20%", left: 0, right: 0, bottom: 0, display: "flex", gap: 0 }}>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, borderRight: "2px solid rgba(255,100,0,0.4)", background: playerWins ? "linear-gradient(160deg,rgba(5,46,22,0.85),rgba(20,83,45,0.7))" : "linear-gradient(160deg,rgba(20,5,5,0.85),rgba(40,10,10,0.7))", padding: "0 12px" }}>
      ...
      {phase === "atk" && (
        <div style={{ textAlign: "center", animation: "scaleIn 0.5s cubic-bezier(0.34,1.56,0.64,1)" }}>
          <div style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", letterSpacing: 2, marginBottom: 4 }}>攻撃力</div>
          <div style={{ fontSize: 56, fontWeight: "900", color: playerWins ? "#fbbf24" : "#f87171", textShadow: playerWins ? "0 0 30px #f59e0b, 0 0 60px #d97706, 4px 4px 0 #78350f" : "0 0 20px #ef4444, 2px 2px 0 #7f1d1d", lineHeight: 1 }}>{playerAtk}</div>
        </div>
      )}

      {/* ATK bar */}
      {phase === "atk" && (
        <div style={{ width: "80%", animation: "fadeInUp 0.3s ease 0.3s both" }}>
          <div style={{ height: 10, background: "#1a1a1a", borderRadius: 5, overflow: "hidden", border: "1px solid rgba(255,255,255,0.1)" }}>
            <div style={{ height: "100%", width: `${playerPct}%`, background: playerWins ? "linear-gradient(90deg,#22c55e,#4ade80,#86efac)" : "linear-gradient(90deg,#dc2626,#ef4444,#f87171)", borderRadius: 5, transition: "width 0.8s cubic-bezier(0.4,0,0.2,1) 0.3s", boxShadow: playerWins ? "0 0 8px #4ade80" : "0 0 8px #ef4444" }} />
          </div>
        </div>
      )}

      {phase === "result" && (
        <div style={{ fontWeight: "900", fontSize: 36, color: playerWins ? "#4ade80" : "#f87171", textShadow: playerWins ? "0 0 24px #4ade80, 0 0 48px #16a34a, 0 0 80px #4ade8066" : "0 0 20px #f87171, 0 0 40px #dc2626", animation: "scaleIn 0.5s cubic-bezier(0.34,1.56,0.64,1)", letterSpacing: 2 }}>
          {playerWins ? "勝ち！" : "負け…"}
        </div>
      )}
      {phase === "result" && playerWins && (
        <div style={{ animation: "fadeInUp 0.3s ease 0.2s both", textAlign: "center" }}>
          {["★", "✦", "✸", "★"].map((s, i) => (
            <span key={i} style={{ fontSize: 14, color: "#fbbf24", animation: `starFloat ${0.8 + i * 0.15}s ease-out ${i * 0.1}s both`, display: "inline-block", margin: "0 3px" }}>{s}</span>
          ))}
        </div>
      )}
    </div>
    ...
    {/* Center divider */}
    <div style={{ width: 3, background: "linear-gradient(180deg,transparent,#f97316,#fbbf24,#f97316,transparent)", boxShadow: "0 0 16px #f97316, 0 0 32px rgba(249,115,22,0.4)", animation: "pulse 0.3s infinite", flexShrink: 0 }} />
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, background: !playerWins ? "linear-gradient(160deg,rgba(5,46,22,0.85),rgba(20,83,45,0.7))" : "linear-gradient(160deg,rgba(20,5,5,0.85),rgba(40,10,10,0.7))", padding: "0 12px" }}>
      ...
      {phase === "atk" && (
        <div style={{ textAlign: "center", animation: "scaleIn 0.5s cubic-bezier(0.34,1.56,0.64,1) 0.1s both" }}>
          <div style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", letterSpacing: 2, marginBottom: 4 }}>攻撃力</div>
          <div style={{ fontSize: 56, fontWeight: "900", color: !playerWins ? "#fbbf24" : "#f87171", textShadow: !playerWins ? "0 0 30px #f59e0b, 0 0 60px #d97706, 4px 4px 0 #78350f" : "0 0 20px #ef4444, 2px 2px 0 #7f1d1d", lineHeight: 1 }}>{enemyAtk}</div>
        </div>
      )}

      {phase === "atk" && (
        <div style={{ width: "80%", animation: "fadeInUp 0.3s ease 0.4s both" }}>
          <div style={{ height: 10, background: "#1a1a1a", borderRadius: 5, overflow: "hidden", border: "1px solid rgba(255,255,255,0.1)" }}>
            <div style={{ height: "100%", width: `${enemyPct}%`, background: !playerWins ? "linear-gradient(90deg,#22c55e,#4ade80,#86efac)" : "linear-gradient(90deg,#dc2626,#ef4444,#f87171)", borderRadius: 5, transition: "width 0.8s cubic-bezier(0.4,0,0.2,1) 0.4s", boxShadow: !playerWins ? "0 0 8px #4ade80" : "0 0 8px #ef4444" }} />
          </div>
        </div>
      )}

      {phase === "result" && (
        <div style={{ fontWeight: "900", fontSize: 36, color: !playerWins ? "#4ade80" : "#f87171", textShadow: !playerWins ? "0 0 24px #4ade80, 0 0 48px #16a34a, 0 0 80px #4ade8066" : "0 0 20px #f87171, 0 0 40px #dc2626", animation: "scaleIn 0.5s cubic-bezier(0.34,1.56,0.64,1) 0.08s both", letterSpacing: 2 }}>
          {!playerWins ? "勝ち！" : "負け…"}
        </div>
      )}
    </div>
...
// スカウターカウンター：数値がランダムスクランブルされながら最終値に近づく
const ScouterCounter = ({ target, isText }) => {
...
step++;
const revealed = target.slice(0, Math.floor((step / 12) * target.length));
const scrambled = target.slice(revealed.length).split(””).map(() => chars[Math.floor(Math.random() * chars.length)]).join(””);
setDisplay(revealed + scrambled);
if (step >= 12) clearInterval(id);
}, 100);
return () => clearInterval(id);
} else {
// 数値は0からランダムに跳ねながら目標値に近づく
let step = 0;
const steps = 14;
const id = setInterval(() => {
step++;
const progress = step / steps;
const noise = (1 - progress) * target * 0.4;
const val = Math.floor(target * progress + (Math.random() - 0.5) * noise);
...
// サポートカード効果を先に計算（useState の初期値で使うため）
...
// SSJスタート時の初期HP/ATK
const ssj1Hp  = playerCard.isGotenks ? Math.floor(playerCard.hp  * 1.5) : playerCard.hp;
const ssj1Atk = playerCard.isGotenks ? Math.floor(playerCard.atk * 1.5) : playerCard.atk;
...
// サポートカードカットイン管理（バトル開始時）
const [showSupportCutIn, setShowSupportCutIn] = useState(false);

const [rouletteState, setRouletteState] = useState(null);
// atkRouletteLevel: プレイヤーが攻撃したターンで上昇
// defRouletteLevel: プレイヤーが防御（被攻撃）したターンで上昇
...
const [phase, setPhase] = useState(“waiting”); // 登場演出中はwaitingで待機
...
// サポートカード「怒り」があれば最初から界王拳状態・アンロック済み
...
// 「怒り」サポートでゴテンクスはSSJ状態からスタート
const [gotenksLevel, setGotenksLevel] = useState(startsWithSSJ ? 1 : 0);
const gotenksLevelRef = useRef(startsWithSSJ ? 1 : 0);
// ゴテンクスの実効ATKを計算
const getGotenksAtk = (lv) => {
if (!isGotenks) return playerCard.atk;
if (lv === 1) return Math.floor(playerCard.atk * 1.5);
if (lv === 2) return playerCard.atk * 2;
return playerCard.atk;
};
// ゴテンクスの変身最大HP（段階ごと）
const getGotenksMaxHp = (lv) => {
if (!isGotenks) return playerCard.hp;
if (lv === 1) return Math.floor(playerCard.hp * 1.5);
if (lv === 2) return playerCard.hp * 2;
return playerCard.hp;
};
// ROUND表示 → FIGHT! 表示
const [roundPhase, setRoundPhase] = useState(“done”); // バトル開始時は非表示
...
const [playerKiAnim, setPlayerKiAnim] = useState(false);
const [enemyKiAnim, setEnemyKiAnim]   = useState(false);
const [playerVisible, setPlayerVisible] = useState(false);
const [enemyVisible, setEnemyVisible]   = useState(false);
// プレイヤーHP最大値（サポート効果適用後に更新）
const [playerMaxHp, setPlayerMaxHp] = useState(playerCard.hp);
// バトル中のATK現在値（サポート効果後に更新）
// startsWithSSJ の場合はゴテンクスSSJ×1.5で初期化（演出後に反映されるが、ref/stateは最初から正しい値にしておく）
const [playerCurrentAtk, setPlayerCurrentAtk] = useState(playerCard.atk);
// ズーム表示
const [zoomTarget, setZoomTarget] = useState(null); // “player”|“enemy”|null
...
const t4 = setTimeout(() => {
setRoundPhase(“done”); setShowFight(false);
setPhase(“choose”); setTimer(15); setTimerActive(true);
}, 3600);
return () => { clearTimeout(t1); clearTimeout(t2); clearTimeout(t3); clearTimeout(t4); };
};
...
// ① プレイヤー登場 (1.0s〜)
T(() => { setEnterPhase("player_fly"); setPlayerKiAnim(true); setPlayerVisible(false); }, 1000);
T(() => { setPlayerKiAnim(false); setPlayerVisible(true); setEnterPhase("player_land"); }, 2200);

// ② プレイヤーズーム + HP/ATK表示 (2.8s〜)
T(() => { setZoomTarget("player"); setShowZoomStats(false); }, 2800);
T(() => { setShowZoomStats(true); }, 3300);

if (supportCard) {
  // ③ プレイヤーサポートカード発動 (5.0s〜)
  //   斜め帯カットイン → カード情報表示 → カード消滅 → 数値変化 → 敵登場
  T(() => { setShowSupportBanner(true); }, 5000);
  T(() => { setShowSupportBanner(false); setShowSupportInfo(true); }, 5900);
  // カード情報を消す（8.0s）→ ズームに戻る
  T(() => { setShowSupportInfo(false); }, 8000);

  // カード消滅後にステータス変動（8.3s〜）
  T(() => {
    if (startsWithKaioken) {
      // 界王拳：ATK×1.5をズームとバーに反映
      const kaiokenAtk = Math.floor(playerCard.atk * 1.5);
      setZoomPlayerAtk(kaiokenAtk);
      setPlayerCurrentAtk(kaiokenAtk);
      setTimeout(() => setZoomPlayerStatus("界王拳🔥"), 300);
    } else if (startsWithSSJ) {
      // ゴテンクスSSJスタート：ATK×1.5、HP×1.5
      const ssjAtk = Math.floor(playerCard.atk * 1.5);
      const ssjHp  = Math.floor(playerCard.hp  * 1.5);
      const steps = 20;
      for (let i = 1; i <= steps; i++) {
        setTimeout(() => {
          const p = i / steps;
          setZoomPlayerHp(Math.round(playerCard.hp + (ssjHp - playerCard.hp) * p));
          setZoomPlayerAtk(Math.round(playerCard.atk + (ssjAtk - playerCard.atk) * p));
        }, i * 50);
      }
      setPlayerHp(ssjHp); playerHpRef.current = ssjHp;
      setPlayerMaxHp(ssjHp); setPlayerDisplayHp(ssjHp);
      setPlayerCurrentAtk(ssjAtk);
      gotenksLevelRef.current = 1; setGotenksLevel(1);
      setTimeout(() => setZoomPlayerStatus("⚡ スーパーサイヤ人"), 1100);
    } else if (startsWithBoost) {
      // 潜在能力の解放：HP・ATKをカウントアップ
      const fromHp  = playerCard.hp;
      const toHp    = boostedHp;
      const fromAtk = playerCard.atk;
      const toAtk   = boostedAtk;

      // ズーム表示の数値をカウントアップ（20ステップ×50ms = 1秒）
      const steps = 20;
      for (let i = 1; i <= steps; i++) {
        setTimeout(() => {
          const p = i / steps;
          setZoomPlayerHp(Math.round(fromHp + (toHp - fromHp) * p));
          setZoomPlayerAtk(Math.round(fromAtk + (toAtk - fromAtk) * p));
        }, i * 50);
      }

      // 実際のHP/ATK stateとHPバーも同時に更新
      setPlayerHp(toHp);
      playerHpRef.current = toHp;
      setPlayerMaxHp(toHp);
      setPlayerCurrentAtk(toAtk);
      const startTime = performance.now();
      const dur = 1000;
      const tick = (now) => {
        const prog = Math.min(1, (now - startTime) / dur);
        setPlayerDisplayHp(Math.round(fromHp + (toHp - fromHp) * prog));
        if (prog < 1) requestAnimationFrame(tick);
        else setPlayerDisplayHp(toHp);
      };
      requestAnimationFrame(tick);

      // ステータスバッジは1秒後に表示
      setTimeout(() => {
        setZoomPlayerStatus(`HP+${supportCard.hpBoost} / ATK+${supportCard.atkBoost}`);
      }, 1100);
    }
  }, 8300);

  // 変動を見せた後（2秒確保）→ ズーム解除して敵登場へ（10.5s〜）
  T(() => { setZoomTarget(null); setShowZoomStats(false); }, 10500);

  // ④ 敵登場 (11.0s〜)
  T(() => { setEnterPhase("enemy_fly"); setEnemyKiAnim(true); setEnemyVisible(false); }, 11000);
  T(() => { setEnemyKiAnim(false); setEnemyVisible(true); setEnterPhase("enemy_land"); }, 12200);

  // ⑤ 敵ズーム + HP/ATK表示 (12.8s〜)
  T(() => { setZoomTarget("enemy"); setShowZoomStats(false); }, 12800);
  T(() => { setShowZoomStats(true); }, 13300);
  T(() => { setZoomTarget(null); setShowZoomStats(false); setEnterPhase("done"); }, 14800);

  // ⑥ ROUND1 (15.1s〜)
  T(() => { startRound1Sequence(); }, 15100);

} else {
  // サポートなし：プレイヤーズーム終了 → 敵登場 → 敵ズーム → ROUND1
  T(() => { setZoomTarget(null); setShowZoomStats(false); setEnterPhase("enemy_fly"); setEnemyKiAnim(true); setEnemyVisible(false); }, 5000);
  T(() => { setEnemyKiAnim(false); setEnemyVisible(true); setEnterPhase("enemy_land"); }, 6200);
  T(() => { setZoomTarget("enemy"); setShowZoomStats(false); }, 6800);
  T(() => { setShowZoomStats(true); }, 7300);
  T(() => { setZoomTarget(null); setShowZoomStats(false); setEnterPhase("done"); }, 8800);
  T(() => { startRound1Sequence(); }, 9100);
}
...
clearInterval(rouletteCountdownRef.current);
setRouletteCountdown(0);
...
let move;
if (isSpecial) {
move = attacker === “player” ? (playerCard.isGoku ? MOVES.rock_kamehameha : MOVES.rock_punch) : MOVES.rock_punch;
} else {
move = (pendingMove && pendingMove.id === “rock_kamehameha”) ? (attacker === “player” ? MOVES.paper_punch : MOVES.rock_punch) : pendingMove;
}
...
```
  // ルーレットLv更新：攻撃した側のatkLv・防御した側のdefLv を独立して上げる
  if (attacker === "player") {
    // プレイヤー攻撃 → プレイヤーのatkLv上昇、敵のdefLv上昇
    const nAtk = Math.min(3, atkRouletteLevelRef.current + 1);
    atkRouletteLevelRef.current = nAtk; setAtkRouletteLevel(nAtk);
    const nEDef = Math.min(3, enemyDefRouletteLevelRef.current + 1);
    enemyDefRouletteLevelRef.current = nEDef; setEnemyDefRouletteLevel(nEDef);
  } else {
    // 敵攻撃 → 敵のatkLv上昇、プレイヤーのdefLv上昇
    const nEAtk = Math.min(3, enemyAtkRouletteLevelRef.current + 1);
    enemyAtkRouletteLevelRef.current = nEAtk; setEnemyAtkRouletteLevel(nEAtk);
    const nDef = Math.min(3, defRouletteLevelRef.current + 1);
    defRouletteLevelRef.current = nDef; setDefRouletteLevel(nDef);
  }
  ...
  if (isFirstWin && attacker === "player") {
    ...
    setTimeout(() => { doAttack(attacker, move, finalDmg, pendingHand, pendingEHand, pendingIsKaioken); }, 100);
  }
}, 1000);
...
atkTimerRef2.current = setInterval(() => {
  atkIdxRef.current = (atkIdxRef.current + 1) % ROULETTE_TOTAL;
  setRouletteState(prev => prev && !prev.atkStopped ? { ...prev, atkIdx: atkIdxRef.current } : prev);
}, 30);
clearInterval(defTimerRef2.current);
defTimerRef2.current = setInterval(() => {
  defIdxRef.current = (defIdxRef.current + 1) % ROULETTE_TOTAL;
  setRouletteState(prev => prev && !prev.defStopped ? { ...prev, defIdx: defIdxRef.current } : prev);
}, 30);

// ===== カウントダウン（7秒）=====
setRouletteCountdown(7);
clearInterval(rouletteCountdownRef.current);
rouletteCountdownRef.current = setInterval(() => {
  setRouletteCountdown(prev => {
    if (prev <= 1) {
      clearInterval(rouletteCountdownRef.current);
      return 0;
    }
...
if (attacker === "player") {
  ...
  setTimeout(() => {
    ...
    setRouletteState(prev => {
      if (!prev || prev.defStopped) return prev;
      return { ...prev, defStopped: true, defResult: prev.defSlots[defIdxRef.current] };
    });
  }, enemyAutoStop);
  // プレイヤー攻撃ルーレットは7秒で強制停止
  setTimeout(() => {
    clearInterval(atkTimerRef2.current);
    clearInterval(rouletteCountdownRef.current);
    setRouletteCountdown(0);
    setRouletteState(prev => {
      if (!prev || prev.atkStopped) return prev;
      return { ...prev, atkStopped: true, atkResult: prev.atkSlots[atkIdxRef.current] };
    });
  }, 7000);
} else {
  // 敵攻撃ルーレットは1〜3秒でランダム自動停止
  const enemyAutoStop = 1000 + Math.random() * 2000;
  setTimeout(() => {
    clearInterval(atkTimerRef2.current);
    setRouletteState(prev => {
...
setPlayerMeter(0); setEnemyMeter(0);
playerMeterRef.current = 0; enemyMeterRef.current = 0;
playerTapCountRef.current = 0; setKiryokuTimer(5); setShowAtkBonus(false);
setKiryokuPhase(“announce”);
setTimeout(() => {
setKiryokuPhase(“smash”);
const TARGET_ENEMY = 40 + Math.random() * 10;
let ep = 0;
enemySmashRef.current = setInterval(() => {
const rem = TARGET_ENEMY - ep; if (rem <= 0) { clearInterval(enemySmashRef.current); return; }
ep = Math.min(TARGET_ENEMY, ep + Math.max(0.2, rem * 0.08 + Math.random() * 1.5));
setEnemyMeter(Math.min(100, ep));
}, 150);
...
setTimeout(() => {
setDragonBurstPhase(“clash”);
setPlayerAnim(“dash”); setEnemyAnim(“dash”);
setPlayerClashOffset(120); setEnemyClashOffset(-120);
...
if (curLv >= 2) return;
const newLv = curLv + 1;
const lvName = newLv === 1 ? “⚡ スーパーサイヤ人！” : “⚡⚡ スーパーサイヤ人3！！”;
setBattleLog(l => […l, `ゴテンクス ${lvName}`]);
setTimeout(() => {
...
setPlayerMaxHp(newMax); setPlayerDisplayHp(newHp);
const newAtk = getGotenksAtk(newLv);
setPlayerCurrentAtk(newAtk);
gotenksLevelRef.current = newLv;
setGotenksLevel(newLv);
}, 300);
}, []);

const handleDragonBurstResult = useCallback((result, hand, eHand) => {
const currentMultiplier = dragonBurstMultiplierRef.current;
// ゴテンクス現在ATK（変身レベル考慮）＋気力ボーナス
const basePlayerAtk = isGotenks
? getGotenksAtk(gotenksLevelRef.current)
: playerCurrentAtk;
// ステータスバーと同一の計算：kaioken中は×1.5
const playerEffAtk = kaiokenActiveRef.current
? Math.floor((basePlayerAtk + (atkBonus?.player || 0)) * 1.5)
: (basePlayerAtk + (atkBonus?.player || 0));

```
if (result === "draw") {
  const newMult = Math.min(2.0, currentMultiplier + 0.5);
  setBattleLog(l => [...l, `ドラゴンバースト継続！ ATK×${newMult.toFixed(1)}`]);
  setPlayerHand(null); setEnemyHand(null); setJankenResult(null); setJankenResultLabel(null);
  setPhase("attack"); setTimerActive(false);
  if (newMult >= 2.0) {
    const playerTotalAtk = playerEffAtk;
    const enemyTotalAtk = enemyData.atk + (atkBonus?.enemy || 0);
    const forcedAttacker = playerTotalAtk >= enemyTotalAtk ? "player" : "enemy";
    const forcedHand = forcedAttacker === "player" ? hand : eHand;
    const forcedEHand = forcedAttacker === "player" ? eHand : hand;
    ...
    const baseAtk = forcedAttacker === "player"
      ? Math.floor(playerTotalAtk * newMult)
      : Math.floor(enemyTotalAtk * newMult);
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        const isFirstWin200 = forcedAttacker === "player" && playerCard.isGoku
          && kaiokenUnlockedRef.current && !kaiokenActiveRef.current;
        // ゴテンクス200%強制勝利でも変身チェック
        if (isGotenks && forcedAttacker === "player") applyGotenksTransform();
        startAttackRoulette(forcedAttacker, move, baseAtk, forcedHand, forcedEHand, kaiokenActiveRef.current, isFirstWin200);
      }, 2000);
    }, 2200);
  } else {
    setTimeout(() => startDragonBurst(newMult), 500);
  }
  return;
}
...
const move = MOVES[moveId];
// playerEffAtk は既に kaioken 倍率込みなのでそのまま使う
const baseAtk = attacker === "player"
  ? Math.floor(playerEffAtk * currentMultiplier)
  : Math.floor((enemyData.atk + (atkBonus?.enemy || 0)) * currentMultiplier);
const isFirstWinDB = attacker === "player" && playerCard.isGoku
  && kaiokenUnlockedRef.current && !kaiokenActiveRef.current;
// ゴテンクスドラゴンバースト勝利でも変身
...
dragonBurstMultiplierRef.current = 1; setDragonBurstMultiplier(1);
setTimeout(() => {
  const latestBase = playerCard.isGotenks ? getGotenksAtk(gotenksLevelRef.current) : (isGotenks ? getGotenksAtk(gotenksLevelRef.current) : playerCurrentAtk);
  const latestWithBonus = latestBase + (atkBonus?.player || 0);
  const latestEff = kaiokenActiveRef.current ? Math.floor(latestWithBonus * 1.5) : latestWithBonus;
  const latestBaseAtk = attacker === "player" ? Math.floor(latestEff * currentMultiplier) : Math.floor((enemyData.atk + (atkBonus?.enemy || 0)) * currentMultiplier);
  startAttackRoulette(attacker, move, latestBaseAtk, hand, eHand, kaiokenActiveRef.current, isFirstWinDB);
}, 400);
```

}, [playerCard, enemyData, atkBonus, startDragonBurst, startAttackRoulette, playerCurrentAtk, isGotenks]);

// handleDragonBurstResult を ref 経由で参照して循環依存を解消
...
const effectivePlayerAtk = (kaiokenActiveRef.current && attacker === “player”)
? Math.floor(rawPlayerAtk * 1.5) : rawPlayerAtk;
const baseAtk = attacker === “player” ? effectivePlayerAtk : enemyData.atk + atkBonus.enemy;
setCurrentMove(pendingMove);
const isFirstWin = result === “win” && playerCard.isGoku
&& kaiokenUnlockedRef.current && !kaiokenActiveRef.current;

```
  // ゴテンクス変身チェック（結果確定時）
  if (isGotenks && result === "win" && attacker === "player") {
    applyGotenksTransform();
  }

  // ルーレット開始前にATKを再計算（変身チェック後）
  setTimeout(() => {
    // 変身が反映されるまで少し待つ（300ms後に変身するため400ms後に計算）
...
const tick = (now) => {
const progress = Math.min(1, (now - startTime) / 1500);
setter(Math.max(toHp, fromHp - dmg * progress));
if (progress < 1) drainRafRef.current = requestAnimationFrame(tick);
else { setter(toHp); onDone(); }
};
...
```
// 界王拳中にプレイヤーが攻撃 → 自分のHPを200消費（HP最低1）
...
const afterHit = (frozenAnim, hitTarget) => {
  ...
  setTimeout(() => {
    ...
    // 界王拳の自傷ダメージ：相手HPドレインと同時に適用（HPは必ず1残す）
    if (kaiokenSelfDmg > 0) {
      const pFrom = playerHpRef.current;
      const pNew = Math.max(1, pFrom - kaiokenSelfDmg);
      setPlayerHp(pNew); playerHpRef.current = pNew;
      drainHp("player", pFrom, pNew, () => {}); // 相手ドレインと並行して流す
    ...
    drainHp(hitTarget, fromHp, newHp, () => {
      if (newHp <= 0) {
        if (hitTarget === "enemy") {
          setPlayerAnim(isKaioken && playerCard.isGoku ? "kaioken_idle" : "win");
          setEnemyAnim("lose");
          setTimeout(() => onEnd(true), 1500);
          setPhase("result");
        } else {
          // プレイヤーKO → イベントカード「ナメック星人の力」チェック
          const canRevive = eventCard?.effect === "revive_half"
            && !eventCardUsedRef.current
            && turn <= 5;
          if (canRevive) {
            eventCardUsedRef.current = true;
            setShowEventCutIn(true);
            setEventCutInDone(() => () => {
              const reviveHp = Math.floor(playerCard.hp / 2);
              setPlayerHp(reviveHp); playerHpRef.current = reviveHp;
              setPlayerDisplayHp(reviveHp);
              setBattleLog(l => [...l, `ナメック星人の力発動！ HP ${reviveHp} 回復！`]);
              setPlayerAnim(isKaioken && playerCard.isGoku ? "kaioken_idle" : "idle");
              setEnemyAnim("idle");
              setDamageNum(null); setCurrentMove(null);
              nextTurn(isKaioken, turn + 1);
            });
          } else {
            setEnemyAnim("win"); setPlayerAnim("lose");
        ...
        if (isGotenks && attacker === "enemy" && gotenksLevelRef.current > 0) {
          const curLv = gotenksLevelRef.current;
          const newLv = curLv - 1;
          const lvName = newLv === 0 ? "ノーマル" : "スーパーサイヤ人";
          setBattleLog(l => [...l, `変身解除！ ${lvName}に戻った！`]);
          const oldMax = getGotenksMaxHp(curLv);
          const newMax = getGotenksMaxHp(newLv);
          // HP割合を保持して新しいステータスに変換
          const ratio = playerHpRef.current / oldMax;
          const newHp = Math.max(1, Math.floor(newMax * ratio));
          setPlayerHp(newHp); playerHpRef.current = newHp;
          setPlayerMaxHp(newMax);
          setPlayerDisplayHp(newHp);
          setPlayerCurrentAtk(getGotenksAtk(newLv));
          gotenksLevelRef.current = newLv;
          setGotenksLevel(newLv);
        }
...
if (needsDash) {
  ...
  setTimeout(() => {
    if (move.id === "scissors_kick") setPlayerAnim("kick"); else setPlayerAnim("punch");
    setEnemyAnim("hit"); setDamageNum(dmg); setDamagePos("enemy");
    setBattleLog(l => [...l, `T${turn}: ${playerCard.name}の${move.name}！ ${dmg}ダメージ！`]);
  }, 1100);
...
setRoundPhase(“round”); setShowRoundNum(roundNum); setRoundFadeOut(false);
setShowFight(false);
...
setPlayerHand(null); setEnemyHand(null); setJankenResult(null);
setTurn(nextTurnNum);
// HP半分以下チェック → 界王拳アンロック
if (playerCard.isGoku && !kaiokenUnlockedRef.current && playerHpRef.current <= boostedHp / 2) {
kaiokenUnlockedRef.current = true;
setKaiokenUnlocked(true);
setBattleLog(l => […l, `HP半分以下！界王拳が使えるようになった！`]);
}

```
const startRound = () => {
  if (nextTurnNum === 4 && !kiryokuUsedRef.current) {
    kiryokuUsedRef.current = true;
    startKiryoku(bonus => { setAtkBonus(bonus); setPhase("choose"); setTimer(15); setTimerActive(true); });
  } else { setPhase("choose"); setTimer(15); setTimerActive(true); }
};

// ROUND N テキスト表示してから次ターン開始
...
```
  {/* イベントカードカットイン（バトル中） */}
  {showEventCutIn && eventCard && (
    <CardCutIn
      card={eventCard}
      cardType="event"
      onDone={() => {
        setShowEventCutIn(false);
        if (eventCutInDone) { eventCutInDone(); setEventCutInDone(null); }
      }}
    />
  )}
  ...
  <div style={{ padding: "10px 14px 8px", position: "relative", zIndex: 2, background: "rgba(0,0,0,0.55)", backdropFilter: "blur(3px)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 64px 1fr", gap: 10, alignItems: "center" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
        <div style={{ width: 50, height: 50, borderRadius: "50%", background: phase === "choose" ? (timer <= 5 ? "radial-gradient(circle,#3d0000,#1a0000)" : "radial-gradient(circle,#0d1a2e,#060d1a)") : "radial-gradient(circle,#0d0d0d,#060606)", border: phase === "choose" ? (timer <= 5 ? "2px solid #ef4444" : "2px solid #3b82f6") : "2px solid #1f2937", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", boxShadow: phase === "choose" && timer <= 5 ? "0 0 12px rgba(239,68,68,0.6)" : "none", transition: "all 0.3s", flexShrink: 0 }}>
          <div key={timer} style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: timer >= 10 ? 18 : 22, color: phase === "choose" ? (timer <= 5 ? "#ef4444" : "#ffffff") : "#374151", lineHeight: 1, animation: phase === "choose" && timer <= 3 ? "pulse 0.4s infinite" : "none" }}>{phase === "choose" ? timer : "—"}</div>
    ...
    {phase === "choose" && (
      <div style={{ marginTop: 8, height: 3, background: "rgba(255,255,255,0.08)", borderRadius: 2 }}>
        <div style={{ height: "100%", width: `${(timer / 15) * 100}%`, background: timer <= 5 ? "linear-gradient(90deg,#ef4444,#ff6b6b)" : "linear-gradient(90deg,#3b82f6,#60a5fa)", borderRadius: 2, transition: "width 1s linear" }} />
      </div>
    )}
    {/* ===== ステータスバー ===== */}
    <div style={{ display: "grid", gridTemplateColumns: "1fr 64px 1fr", gap: 10, marginTop: 4 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
        <div style={{ fontSize: 10, color: "#f87171", fontFamily: "monospace", fontWeight: "700", background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 4, padding: "1px 5px" }}>
          ATK {kaiokenActive
            ? Math.floor((playerCurrentAtk + atkBonus.player) * 1.5)
            : (playerCurrentAtk + atkBonus.player)}
        </div>
        {kaiokenActive && (
          <div style={{ fontSize: 10, color: "#ff4400", fontFamily: "monospace", fontWeight: "900", background: "rgba(255,68,0,0.15)", border: "1px solid rgba(255,68,0,0.5)", borderRadius: 4, padding: "1px 5px", animation: "pulse 0.6s infinite", textShadow: "0 0 6px #ff4400" }}>
            界王拳🔥
          </div>
        )}
        {isGotenks && gotenksLevel === 1 && (
          <div style={{ fontSize: 9, color: "#fbbf24", fontFamily: "monospace", fontWeight: "900", background: "rgba(251,191,36,0.15)", border: "1px solid rgba(251,191,36,0.5)", borderRadius: 4, padding: "1px 5px", animation: "pulse 0.8s infinite", textShadow: "0 0 6px #fbbf24" }}>
            ⚡SSJ
          </div>
        )}
        {isGotenks && gotenksLevel === 2 && (
          <div style={{ fontSize: 9, color: "#a78bfa", fontFamily: "monospace", fontWeight: "900", background: "rgba(167,139,250,0.15)", border: "1px solid rgba(167,139,250,0.5)", borderRadius: 4, padding: "1px 5px", animation: "pulse 0.4s infinite", textShadow: "0 0 8px #a78bfa" }}>
            ⚡⚡SSJ3
          </div>
        )}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" }}>
        <div style={{ fontSize: 10, color: "#f87171", fontFamily: "monospace", fontWeight: "700", background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 4, padding: "1px 5px" }}>
          ATK {enemyData.atk + atkBonus.enemy}
        </div>
      </div>
    </div>
  </div>

  {/* ===== キャラズーム + HP/ATK表示オーバーレイ ===== */}
  {zoomTarget && (
    <div style={{
      ...
      {/* ズームされたキャラ（左寄り）＋ 右横にステータス */}
      <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
        {/* キャラ拡大 */}
        <div style={{
          transform: "scale(2.2)",
          transformOrigin: "center bottom",
          animation: "scouterZoomIn 0.5s cubic-bezier(0.34,1.56,0.64,1) both",
        }}>
          {zoomTarget === "player" ? (
            <CharacterFighter card={playerCard} animState={startsWithKaioken ? "kaioken_idle" : "idle"} size={110} scale={0.85} />
          ) : (
            <CharacterFighter card={enemyData} animState="idle" isEnemy size={110} />
          )}
        </div>
        ...
        {showZoomStats && (
          <div style={{
            ...
            animation: "scouterStatIn 0.4s ease both",
            marginLeft: 16,
          }}>
            {/* キャラ名 */}
            <div style={{
              fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900",
              fontSize: 13, color: "#fff",
              textShadow: `0 0 12px ${zoomTarget === "player" ? (playerCard.color || "#f59e0b") : (enemyData.color || "#ef4444")}`,
              letterSpacing: 2, marginBottom: 4,
              borderLeft: `3px solid ${zoomTarget === "player" ? (playerCard.color || "#f59e0b") : (enemyData.color || "#ef4444")}`,
              paddingLeft: 8,
            }}>
              {zoomTarget === "player" ? playerCard.name : enemyData.name}
            </div>
            {/* HP */}
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <div style={{ fontFamily: "monospace", fontSize: 9, color: "#4ade8088", letterSpacing: 2 }}>HP</div>
              <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 22, color: "#4ade80", textShadow: "0 0 12px #4ade80", letterSpacing: 1 }}>
                {zoomTarget === "player" ? zoomPlayerHp : enemyData.hp}
              </div>
            </div>
            {/* ATK */}
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <div style={{ fontFamily: "monospace", fontSize: 9, color: "#f8717188", letterSpacing: 2 }}>ATK</div>
              <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 22, color: "#f87171", textShadow: "0 0 12px #ef4444", letterSpacing: 1 }}>
                {zoomTarget === "player" ? zoomPlayerAtk : enemyData.atk}
              </div>
            </div>
            {/* 状態バッジ（界王拳など） */}
            {zoomTarget === "player" && zoomPlayerStatus && (
              <div style={{
                fontFamily: "monospace", fontWeight: "900", fontSize: 11,
                color: "#ff4400", background: "rgba(255,68,0,0.15)",
  ...
  {showSupportBanner && supportCard && (
    <div style={{
      position: "fixed", inset: 0, zIndex: 82, pointerEvents: "none",
      display: "flex", alignItems: "center", justifyContent: "center",
      overflow: "hidden",
    }}>
      {/* 斜め帯 */}
      <div style={{
        ...
        background: `linear-gradient(105deg, transparent 20%, ${supportCard.color}dd 20%, ${supportCard.color}dd 80%, transparent 80%)`,
        animation: "supportBannerSlide 0.4s cubic-bezier(0.22,1,0.36,1) both",
      }} />
      <div style={{
        position: "relative", zIndex: 1, textAlign: "center",
        animation: "supportBannerSlide 0.4s cubic-bezier(0.22,1,0.36,1) 0.05s both",
      }}>
        <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 16, color: "#000", letterSpacing: 4 }}>
          ⚡ サポートカード発動！
        </div>
        <div style={{ fontFamily: "'Courier New',monospace", fontWeight: "900", fontSize: 22, color: "#000", letterSpacing: 2, marginTop: 4 }}>
          {supportCard.name}
        </div>
      </div>
    </div>
  ...
  {showSupportInfo && supportCard && (
    <div style={{
      ...
      {/* カードイラスト（カード形式） */}
      <div style={{
        width: 110, height: 154, borderRadius: 8,
        background: `linear-gradient(160deg,${supportCard.color}18,${supportCard.color}08,#0a0010)`,
        border: `2px solid ${supportCard.color}`,
        boxShadow: `0 0 24px ${supportCard.color}88`,
        display: "flex", flexDirection: "column", alignItems: "center", padding: "4px 3px 4px",
        position: "relative", overflow: "hidden",
        animation: "cutInCardIn 0.5s cubic-bezier(0.34,1.56,0.64,1) both",
        flexShrink: 0,
      }}>
        ...
        <div style={{ marginTop: 14, flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ fontSize: 40, filter: `drop-shadow(0 0 10px ${supportCard.color})` }}>{supportCard.illustSymbol}</div>
        </div>
      ...
      {/* 効果テキスト＋数値変化 */}
      <div style={{
        display: "flex", flexDirection: "column", gap: 10, maxWidth: 180,
        ...
        {zoomPlayerStatus && (
          <div style={{
            ...
            <div style={{ fontFamily: "monospace", fontSize: 9, color: "#6b7280", letterSpacing: 2 }}>— 効果付与 —</div>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <span style={{ fontSize: 9, color: "#f8717188", fontFamily: "monospace" }}>ATK</span>
              <span style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 16, color: "#f87171", textShadow: "0 0 8px #f87171" }}>{zoomPlayerAtk}</span>
            </div>
            <div style={{ fontFamily: "monospace", fontWeight: "900", fontSize: 12, color: "#ff4400", textShadow: "0 0 8px #ff4400", animation: "pulse 0.6s infinite" }}>{zoomPlayerStatus}</div>
          </div>
        )}
      </div>
    </div>
  ...
  <div style={{ flex: 1, display: "flex", alignItems: "flex-end", justifyContent: "space-around", padding: "0 10px 20px", position: "relative", minHeight: 220 }}>
    ...
    {roundPhase === "round" && (
      <div style={{ position: "absolute", inset: 0, zIndex: 20, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
        <div style={{
          textAlign: "center",
          animation: roundFadeOut ? "fightOut 0.5s ease forwards" : "fightIn 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards",
        }}>
          <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 18, color: "#60a5fa", letterSpacing: 8, textShadow: "0 0 16px #3b82f6", marginBottom: 4 }}>ROUND</div>
          <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 80, color: "#ffd700", textShadow: "0 0 40px #ffd700, 0 0 80px #f59e0b88, 4px 4px 0 #7c2d12, -2px -2px 0 #000", lineHeight: 1 }}>{showRoundNum}</div>
        </div>
      </div>
    )}
    ...
    {showFight && (
      <div style={{ position: "absolute", inset: 0, zIndex: 20, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
        <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 54, color: "#ffd700", textShadow: "0 0 30px #ffd700, 0 0 60px #f59e0b88, 4px 4px 0 #7c2d12, -1px -1px 0 #000", letterSpacing: 6, animation: fightFadeOut ? "fightOut 0.5s ease forwards" : "fightIn 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>FIGHT!</div>
      </div>
    )}

    {/* ジャンケン残り5秒以下の大カウントダウン */}
    {phase === "choose" && timer > 0 && timer <= 5 && (
      <div style={{ position: "absolute", inset: 0, zIndex: 19, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
        <div key={`jk-cd-${timer}`} style={{
          fontFamily: "'Orbitron','Courier New',monospace",
          fontWeight: "900",
          fontSize: timer <= 3 ? 120 : 96,
          color: timer <= 3 ? "#ef4444" : "#f59e0b",
          textShadow: timer <= 3
            ? "0 0 50px #ef4444, 0 0 100px #dc2626aa, 5px 5px 0 #7f1d1d, -5px -5px 0 #7f1d1d"
            : "0 0 40px #f59e0b, 0 0 80px #d97706aa, 4px 4px 0 #78350f, -4px -4px 0 #78350f",
          lineHeight: 1,
          opacity: 0.85,
          animation: "rouletteCountPop 0.3s cubic-bezier(0.34,1.56,0.64,1) forwards",
        }}>{timer}</div>
      </div>
    )}
    ...
    {/* ダメージテキスト */}
    {dmgText && (
      <div style={{ position: "absolute", inset: 0, zIndex: 18, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
        <div style={{ textAlign: "center", animation: "dmgTextIn 0.35s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>
          <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 58, color: dmgText.target === "enemy" ? "#fbbf24" : "#f87171", textShadow: dmgText.target === "enemy" ? "0 0 24px #f59e0b, 0 0 48px #d97706, 3px 3px 0 #78350f, -2px -2px 0 #78350f" : "0 0 24px #ef4444, 0 0 48px #dc2626, 3px 3px 0 #7f1d1d, -2px -2px 0 #7f1d1d", letterSpacing: 2, lineHeight: 1 }}>{dmgText.amount}</div>
          <div style={{ fontFamily: "'Courier New',monospace", fontWeight: "900", fontSize: 20, color: "#fff", textShadow: "0 0 12px rgba(255,255,255,0.7), 2px 2px 0 rgba(0,0,0,0.8)", letterSpacing: 4, marginTop: 4 }}>ダメージ！</div>
        </div>
      </div>
    )}

    {/* 気力全開 announce */}
    {kiryokuPhase === "announce" && (
      <div style={{ position: "absolute", inset: 0, zIndex: 30, display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg,rgba(0,10,40,0.85),rgba(10,0,40,0.85))", pointerEvents: "none" }}>
        <div style={{ textAlign: "center", animation: "kiryokuIn 0.6s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>
          <div style={{ fontFamily: "'Courier New',monospace", fontWeight: "900", fontSize: 44, color: "#fff", textShadow: "0 0 20px #3b82f6, 0 0 40px #1d4ed8, 0 0 60px #93c5fd, 3px 3px 0 #000", letterSpacing: 2, lineHeight: 1.1, WebkitTextStroke: "1px #60a5fa" }}>気力全開<br />チャンス！</div>
          <div style={{ marginTop: 10, fontSize: 12, color: "#93c5fd", letterSpacing: 6, animation: "pulse 0.6s infinite" }}>⚡ POWER UP ⚡</div>
        </div>
      </div>
    )}

    {/* ドラゴンバースト announce */}
    {dragonBurstPhase === "announce" && (
      <div style={{ position: "absolute", inset: 0, zIndex: 30, display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg,rgba(40,0,0,0.82),rgba(20,0,0,0.88))", pointerEvents: "none" }}>
        <div style={{ textAlign: "center", animation: "dragonBurstIn 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>
          <div style={{ fontFamily: "'Courier New',monospace", fontWeight: "900", fontSize: 46, color: "#fff", textShadow: "0 0 16px #ef4444, 0 0 32px #dc2626, 0 0 64px #f97316, 4px 4px 0 #000", letterSpacing: 2, lineHeight: 1.1, WebkitTextStroke: "2px #fca5a5" }}>ドラゴン<br />バースト！</div>
          {dragonBurstMultiplier > 1 && <div style={{ marginTop: 8, fontSize: 16, fontWeight: "900", color: "#fbbf24", textShadow: "0 0 16px #f59e0b", animation: "pulse 0.4s infinite" }}>ATK × {dragonBurstMultiplier.toFixed(1)}</div>}
        </div>
      </div>
    )}
    ...
    {dragonBurstPhase === "janken" && (
      <div style={{ position: "absolute", top: 10, left: 0, right: 0, display: "flex", justifyContent: "center", pointerEvents: "none", zIndex: 10 }}>
        <div style={{ background: "rgba(0,0,0,0.75)", border: "2px solid #ef4444", borderRadius: 20, padding: "4px 16px", fontFamily: "monospace", fontWeight: "900", fontSize: 13, color: "#fbbf24", textShadow: "0 0 12px #f59e0b", letterSpacing: 2 }}>🔥 ATK × {dragonBurstMultiplier.toFixed(1)}</div>
      </div>
    )}
    ...
    <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", transform: `translateX(${playerOffset + playerClashOffset}px)`, transition: "transform 0.4s ease" }}>
      ...
      {playerKiAnim && (
        <div style={{
          ...
          animation: "kiEnter 1.2s cubic-bezier(0.4,0,0.2,1) forwards",
          pointerEvents: "none", zIndex: 5,
        }}>
          {/* 気の核 */}
          <div style={{ position: "relative", width: 60, height: 60, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <div style={{ position: "absolute", width: 40, height: 40, borderRadius: "50%", background: "radial-gradient(circle, #fff 0%, #fbbf24 35%, #f97316 60%, transparent 100%)", boxShadow: "0 0 30px 10px #fbbf24, 0 0 60px 20px #f59e0b88", animation: "coreFlash 0.15s ease-in-out infinite alternate" }} />
            {/* 気の尾（縦長） */}
            <div style={{ position: "absolute", bottom: "100%", left: "50%", transform: "translateX(-50%)", width: 12, height: 80, background: "linear-gradient(180deg, transparent 0%, #fbbf2466 30%, #fbbf24cc 70%, #fff 100%)", borderRadius: "6px 6px 0 0", filter: "blur(2px)" }} />
            {/* 周囲パーティクル */}
            {[0,60,120,180,240,300].map((deg,i) => (
              <div key={i} style={{ position: "absolute", width: 5, height: 5, borderRadius: "50%", background: ["#fff","#fbbf24","#f97316","#fde68a","#fbbf24","#fff"][i], animation: `particle${i%3} ${0.3+i*0.04}s ease-out infinite ${i*0.03}s`, transform: `rotate(${deg}deg) translateX(${16+i%2*8}px)` }} />
            ))}
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        animation: playerVisible && enterPhase === "player_land" ? "charLand 0.4s cubic-bezier(0.22,1,0.36,1) forwards" : "none",
      }}>
        {playerCard.isGoku
          ? <GokuFighter animState={dragonBurstPhase === "janken" ? "dash" : playerAnim} scale={0.85} />
      ...
      {clashSparks && (dragonBurstPhase === "janken" || dragonBurstPhase === "clash") && (
        <div style={{ position: "relative", width: 70, height: 80, overflow: "visible" }}>
          ...
          {[0, 45, 90, 135, 180, 225, 270, 315].map((deg, i) => (
            <div key={i} style={{ position: "absolute", top: "50%", left: "50%", width: i % 2 === 0 ? 28 : 18, height: 2, transformOrigin: "0 50%", transform: `translate(0,-50%) rotate(${deg}deg)`, background: `linear-gradient(90deg,${i % 3 === 0 ? "#fff" : i % 3 === 1 ? "#fbbf24" : "#f97316"},transparent)`, borderRadius: 2, animation: `sparkRay ${0.2 + i * 0.03}s ease-in-out infinite alternate ${i * 0.02}s` }} />
          ))}
          {[0, 60, 120, 180, 240, 300].map((deg, i) => (
            <div key={i} style={{ position: "absolute", top: "50%", left: "50%", width: 4 + i % 3 * 2, height: 4 + i % 3 * 2, borderRadius: "50%", background: ["#fff", "#fbbf24", "#f97316", "#fde68a", "#ef4444", "#fb923c"][i], animation: `particle${i % 3} ${0.35 + i * 0.06}s ease-out infinite ${i * 0.05}s`, transform: `translate(-50%,-50%) rotate(${deg}deg) translateX(${18 + i % 3 * 8}px)` }} />
          ))}
        </div>
      )}
    ...
    <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", transform: `translateX(${enemyClashOffset}px)`, transition: "transform 0.4s ease" }}>
      ...
      <div style={{
        ...
        animation: enemyVisible && enterPhase === "enemy_land" ? "charLand 0.4s cubic-bezier(0.22,1,0.36,1) forwards" : "none",
      }}>
  ...
  {kiryokuPhase === "smash" && (
    <div style={{ position: "fixed", inset: 0, zIndex: 40, background: "linear-gradient(180deg,rgba(0,5,30,0.95) 0%,rgba(5,0,30,0.98) 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 0, fontFamily: "'Courier New',monospace" }} onTouchStart={handleKiryokuTap} onClick={handleKiryokuTap}>
      ...
      <div key={kiryokuTimer} style={{ fontSize: 32, fontWeight: "900", color: kiryokuTimer <= 2 ? "#ef4444" : "#ffd700", textShadow: `0 0 20px ${kiryokuTimer <= 2 ? "#ef4444" : "#ffd700"}`, marginBottom: 8, animation: kiryokuTimer <= 2 ? "pulse 0.3s infinite" : "none" }}>{kiryokuTimer}</div>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "center", gap: 10, width: "100%", maxWidth: 340, marginBottom: 12 }}>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
          <div style={{ fontSize: 22, fontWeight: "900", color: "#4ade80", textShadow: "0 0 16px #4ade80" }}>{Math.round(playerMeter)}%</div>
          <div style={{ width: 28, height: 120, background: "#0a1208", border: "2px solid #1a3010", borderRadius: 4, overflow: "hidden", position: "relative" }}>
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: `${playerMeter}%`, background: "linear-gradient(180deg,#5eff3a,#22c55e,#16a34a)", transition: "height 0.1s ease" }} />
          </div>
          <div style={{ fontSize: 8, color: "rgba(255,255,255,0.5)" }}>あなた</div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
          <div style={{ fontWeight: "900", fontSize: 18, color: "#ffd700", textShadow: "0 0 16px #ffd700", letterSpacing: 2, animation: "pulse 0.4s infinite", whiteSpace: "nowrap" }}>押しまくれ！！</div>
        </div>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
          <div style={{ fontSize: 22, fontWeight: "900", color: "#f87171", textShadow: "0 0 16px #f87171" }}>{Math.round(enemyMeter)}%</div>
          <div style={{ width: 28, height: 120, background: "#1a0808", border: "2px solid #3f1010", borderRadius: 4, overflow: "hidden", position: "relative" }}>
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: `${enemyMeter}%`, background: "linear-gradient(180deg,#ff6b6b,#ef4444,#dc2626)", transition: "height 0.15s ease" }} />
          </div>
  ...
  {kiryokuPhase === "result_show" && (
    <div style={{ position: "fixed", inset: 0, zIndex: 40, fontFamily: "'Courier New',monospace", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: 0, left: 0, bottom: 0, width: "50%",
        background: playerMeter >= enemyMeter
          ? "linear-gradient(135deg,#1a0e00,#3d2800,#1a0e00)"   /* 勝利=金系 */
          : "linear-gradient(135deg,#0e0020,#200040,#0e0020)",  /* 敗北=紫系 */
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, borderRight: "2px solid rgba(255,255,255,0.15)" }}>
        {/* 背景オーラ */}
        <div style={{ position: "absolute", inset: 0, background: playerMeter >= enemyMeter
        ...
        <div style={{ position: "relative", zIndex: 1, fontWeight: "900", fontSize: 30,
          color: playerMeter >= enemyMeter ? "#fbbf24" : "#a78bfa",
          textShadow: playerMeter >= enemyMeter
            ? "0 0 24px #f59e0b, 0 0 48px #d97706, 0 0 80px #fbbf2466"
            : "0 0 24px #a78bfa, 0 0 48px #7c3aed66",
          animation: "scaleIn 0.5s cubic-bezier(0.34,1.56,0.64,1)", letterSpacing: 2 }}>
          {playerMeter >= enemyMeter ? "勝ち！" : "負け…"}
        </div>
        <div style={{ position: "relative", zIndex: 1, fontSize: 40, fontWeight: "900",
          color: playerMeter >= enemyMeter ? "#fde68a" : "#ddd6fe" }}>
          {Math.round(playerMeter)}<span style={{ fontSize: 20 }}>%</span>
        </div>
        {showAtkBonus && (
          <div style={{ position: "relative", zIndex: 1, animation: "atkUpIn 0.6s cubic-bezier(0.34,1.56,0.64,1) 0.25s both" }}>
            <div style={{
              background: atkBonus.player === 100
                ? "linear-gradient(135deg,#78350f,#d97706,#fbbf24,#d97706,#78350f)"
                : "linear-gradient(135deg,#2e1065,#5b21b6,#7c3aed,#5b21b6,#2e1065)",
              borderRadius: 10, padding: "10px 18px", textAlign: "center",
              boxShadow: atkBonus.player === 100
                ? "0 0 30px #f59e0b, 0 0 60px #d97706aa, inset 0 1px 0 rgba(255,255,255,0.3)"
                : "0 0 24px #7c3aedaa, inset 0 1px 0 rgba(255,255,255,0.2)",
              animation: atkBonus.player === 100 ? "goldShine 1s infinite" : "none",
            }}>
              <div style={{ fontSize: 9, color: atkBonus.player === 100 ? "#fef3c7" : "#ede9fe", letterSpacing: 2 }}>攻撃力</div>
              <div style={{ fontSize: atkBonus.player === 100 ? 36 : 28, fontWeight: "900",
                color: atkBonus.player === 100 ? "#fff" : "#ddd6fe",
                textShadow: atkBonus.player === 100 ? "0 0 20px #fbbf24" : "0 0 12px #a78bfa" }}>
                +{atkBonus.player}
              </div>
              <div style={{ fontSize: 14, fontWeight: "900",
                color: atkBonus.player === 100 ? "#fde68a" : "#c4b5fd", letterSpacing: 4, marginTop: 2 }}>UP!</div>
            </div>
            {/* 勝利時の星エフェクト */}
            {atkBonus.player === 100 && ["★","✦","✸","★","✦"].map((s, i) => (
              <div key={i} style={{ position: "absolute", top: `${-10 + i * 8}px`, left: `${-8 + i * 18}px`,
                fontSize: 10 + i % 3 * 4, color: "#fbbf24",
                animation: `starFloat ${0.8 + i * 0.15}s ease-out ${i * 0.1}s both`,
                pointerEvents: "none" }}>{s}</div>
            ))}
          </div>
        )}
        <div style={{ position: "relative", zIndex: 1, fontSize: 8, color: "rgba(255,255,255,0.35)", letterSpacing: 2 }}>あなた</div>
      </div>

      {/* 敵側パネル */}
      <div style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: "50%",
        background: enemyMeter > playerMeter
          ? "linear-gradient(135deg,#1a0e00,#3d2800,#1a0e00)"   /* 勝利=金系 */
          : "linear-gradient(135deg,#0e0020,#200040,#0e0020)",  /* 敗北=紫系 */
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6 }}>
        <div style={{ position: "absolute", inset: 0, background: enemyMeter > playerMeter
          ? "radial-gradient(ellipse at center, rgba(251,191,36,0.2) 0%, transparent 70%)"
          : "radial-gradient(ellipse at center, rgba(167,139,250,0.18) 0%, transparent 70%)",
          animation: "pulse 1s infinite 0.5s" }} />
        <div style={{ position: "relative", zIndex: 1, fontWeight: "900", fontSize: 30,
          color: enemyMeter > playerMeter ? "#fbbf24" : "#a78bfa",
          textShadow: enemyMeter > playerMeter
            ? "0 0 24px #f59e0b, 0 0 48px #d97706, 0 0 80px #fbbf2466"
            : "0 0 24px #a78bfa, 0 0 48px #7c3aed66",
          animation: "scaleIn 0.5s cubic-bezier(0.34,1.56,0.64,1) 0.08s both", letterSpacing: 2 }}>
          {enemyMeter > playerMeter ? "勝ち！" : "負け…"}
        </div>
        <div style={{ position: "relative", zIndex: 1, fontSize: 40, fontWeight: "900",
          color: enemyMeter > playerMeter ? "#fde68a" : "#ddd6fe" }}>
          {Math.round(enemyMeter)}<span style={{ fontSize: 20 }}>%</span>
        </div>
        {showAtkBonus && (
          <div style={{ position: "relative", zIndex: 1, animation: "atkUpIn 0.6s cubic-bezier(0.34,1.56,0.64,1) 0.35s both" }}>
            <div style={{
              background: atkBonus.enemy === 100
                ? "linear-gradient(135deg,#78350f,#d97706,#fbbf24,#d97706,#78350f)"
                : "linear-gradient(135deg,#2e1065,#5b21b6,#7c3aed,#5b21b6,#2e1065)",
              borderRadius: 10, padding: "10px 18px", textAlign: "center",
              boxShadow: atkBonus.enemy === 100
                ? "0 0 30px #f59e0b, 0 0 60px #d97706aa"
                : "0 0 24px #7c3aedaa",
              animation: atkBonus.enemy === 100 ? "goldShine 1s infinite" : "none",
            }}>
              <div style={{ fontSize: 9, color: atkBonus.enemy === 100 ? "#fef3c7" : "#ede9fe", letterSpacing: 2 }}>攻撃力</div>
              <div style={{ fontSize: atkBonus.enemy === 100 ? 36 : 28, fontWeight: "900",
                color: atkBonus.enemy === 100 ? "#fff" : "#ddd6fe",
                textShadow: atkBonus.enemy === 100 ? "0 0 20px #fbbf24" : "0 0 12px #a78bfa" }}>
                +{atkBonus.enemy}
              </div>
              <div style={{ fontSize: 14, fontWeight: "900",
                color: atkBonus.enemy === 100 ? "#fde68a" : "#c4b5fd", letterSpacing: 4, marginTop: 2 }}>UP!</div>
            </div>
          </div>
        )}
        <div style={{ position: "relative", zIndex: 1, fontSize: 8, color: "rgba(255,255,255,0.35)", letterSpacing: 2 }}>てき</div>
      </div>

      {/* 中央仕切り */}
      <div style={{ position: "absolute", top: 0, bottom: 0, left: "50%", width: 3,
        background: "linear-gradient(180deg,transparent,#fff,rgba(255,255,255,0.8),#fff,transparent)",
        boxShadow: "0 0 16px #fff, 0 0 32px rgba(255,255,255,0.5)",
        transform: "translateX(-50%)", animation: "pulse 0.6s infinite" }} />
    </div>
  )}
  ...
  {rouletteState && (
    <div style={{ position: "fixed", inset: 0, zIndex: 55, display: "flex", flexDirection: "row", fontFamily: "monospace", overflow: "hidden" }}
      ...
      {[true, false].map((isPlayerSide) => {
        ...
        let lv;
        if (isAttack) {
          lv = rouletteState.attacker === "player" ? atkRouletteLevel : enemyAtkRouletteLevel;
        } else {
          lv = rouletteState.attacker === "player" ? enemyDefRouletteLevel : defRouletteLevel;
        }
        ...
        return (
          <div key={String(isPlayerSide)} style={{ width: "50%", flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4, background: isAttack ? "linear-gradient(160deg,#1a0000,#2a0800)" : "linear-gradient(160deg,#00001a,#000a20)", borderRight: isPlayerSide ? `2px solid ${accentColor}44` : "none", padding: "6px 2px", position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse at center,${accentColor}18 0%,transparent 70%)`, pointerEvents: "none" }} />
      ...
      {/* ルーレット残り5秒以下カウントダウン（中央オーバーレイ） */}
      {rouletteCountdown > 0 && rouletteCountdown <= 5 && (
        <div style={{
          position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center",
          pointerEvents: "none", zIndex: 10,
        }}>
          <div key={rouletteCountdown} style={{
            fontFamily: "'Orbitron','Courier New',monospace",
            fontWeight: "900",
            fontSize: rouletteCountdown <= 3 ? 100 : 80,
            color: rouletteCountdown <= 3 ? "#ef4444" : "#fbbf24",
            textShadow: rouletteCountdown <= 3
              ? "0 0 40px #ef4444, 0 0 80px #dc2626, 4px 4px 0 #7f1d1d, -4px -4px 0 #7f1d1d"
              : "0 0 30px #fbbf24, 0 0 60px #f59e0b, 4px 4px 0 #78350f, -4px -4px 0 #78350f",
            lineHeight: 1,
            animation: "rouletteCountPop 0.25s cubic-bezier(0.34,1.56,0.64,1) forwards",
            opacity: 0.92,
          }}>{rouletteCountdown}</div>
        </div>
      )}
    </div>
  )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", padding: "0 8px" }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 7, position: "relative" }}>
          <div style={{ width: 136, display: "flex", justifyContent: "center", position: "relative" }}>
            ...
            {showSet && playerHand === "rock" && <div style={{ position: "absolute", bottom: -8, right: 8, zIndex: 20, pointerEvents: "none", fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 13, color: "#4ade80", textShadow: "0 0 16px #4ade80, 2px 2px 0 #052e16", letterSpacing: 4, animation: "setIn 0.25s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>SET!</div>}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {["scissors", "paper"].map(h => (
              <div key={h} style={{ position: "relative" }}>
                ...
                {showSet && playerHand === h && <div style={{ position: "absolute", bottom: -8, right: -4, zIndex: 20, pointerEvents: "none", fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 13, color: "#4ade80", textShadow: "0 0 16px #4ade80, 2px 2px 0 #052e16", letterSpacing: 4, animation: "setIn 0.25s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>SET!</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 20 }}>
        {[{ hand: playerHand, label: "YOU", myResult: jankenResultLabel }, { hand: enemyHand, label: "ENEMY", myResult: jankenResultLabel === "win" ? "lose" : jankenResultLabel === "lose" ? "win" : jankenResultLabel }].map(({ hand, label, myResult }, i) => (
          <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, minWidth: 90 }}>
            {myResult && <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: myResult === "draw" ? 14 : 18, color: myResult === "win" ? "#4ade80" : myResult === "lose" ? "#ef4444" : "#f59e0b", letterSpacing: 2, animation: `scaleIn 0.3s cubic-bezier(0.34,1.56,0.64,1) ${i * 0.08}s both` }}>
              {myResult === "win" ? "勝ち！" : myResult === "lose" ? "負け…" : "あいこ！"}
            </div>}
        ...
        <div style={{ fontFamily: "'Orbitron','Courier New',monospace", fontWeight: "900", fontSize: 16, color: "#f59e0b", textShadow: "0 0 12px #f59e0b88" }}>VS</div>
      </div>
    </div>
  )}
...
<div style={{ position: “absolute”, inset: 0, background: “radial-gradient(ellipse at 50% 80%, rgba(80,140,255,0.22) 0%, transparent 70%)” }} />
...
<div style={{ position: “absolute”, inset: 0, background: “radial-gradient(ellipse at 50% 80%, rgba(255,80,80,0.2) 0%, transparent 70%)” }} />
...
<div style={{ position: “absolute”, top: “50%”, left: “50%”, transform: “translate(-50%,-50%)”, fontWeight: “900”, fontSize: 68, color: “#fff”, textShadow: “0 0 28px #fff, 0 0 56px rgba(255,255,255,0.5), 4px 4px 0 #000, -2px -2px 0 #000”, letterSpacing: 6, fontFamily: “‘Arial Black’,‘Impact’,sans-serif”, zIndex: 10, animation: “vsTextIn 0.5s cubic-bezier(0.34,1.56,0.64,1) 0.1s both”, WebkitTextStroke: “2px rgba(200,220,255,0.5)”, whiteSpace: “nowrap” }}>VS</div>
</div>
);
};

// ===== RESULT SCREEN (再挑戦削除、ホームへのみ) =====
const ResultScreen = ({ won, playerCard, enemy, onHome }) => (
  ...
  <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: won ? "radial-gradient(ellipse,#052e16 0%,#000 70%)" : "radial-gradient(ellipse,#3b0000 0%,#000 70%)", fontFamily: "monospace", padding: 20 }}>
    {/* Big result text */}
    <div style={{ fontSize: 42, fontWeight: "900", marginBottom: 8, color: won ? "#4ade80" : "#ef4444", textShadow: `0 0 40px ${won ? "#4ade80" : "#ef4444"}, 0 0 80px ${won ? "#4ade8044" : "#ef444444"}`, animation: "scaleIn 0.5s cubic-bezier(0.34,1.56,0.64,1)", letterSpacing: 4 }}>{won ? "WINNER!" : "KO..."}</div>
...
{won && (
  <div style={{ position: "fixed", inset: 0, pointerEvents: "none", overflow: "hidden" }}>
    {Array.from({ length: 20 }).map((_, i) => (
      <div key={i} style={{ position: "absolute", left: `${Math.random() * 100}%`, top: "-20px", width: 8, height: 8, borderRadius: "50%", background: ["#fbbf24", "#4ade80", "#60a5fa", "#f87171", "#a78bfa"][i % 5], animation: `confettiFall ${1.5 + Math.random() * 2}s linear ${Math.random() * 2}s infinite`, opacity: 0.8 }} />
    ))}
  </div>
)}
...
<div style={{ display: "flex", gap: 30, margin: "20px 0", alignItems: "flex-end" }}>
  <div style={{ textAlign: "center" }}>
    <CharacterFighter card={playerCard} animState={won ? "win" : "lose"} size={90} scale={0.55} />
    <div style={{ fontSize: 9, color: "#9ca3af", marginTop: 4 }}>{playerCard.name}</div>
  </div>
  <div style={{ fontSize: 14, color: "#6b7280", marginBottom: 20 }}>VS</div>
  <div style={{ textAlign: "center" }}>
    <CharacterFighter card={enemy} animState={won ? "lose" : "win"} isEnemy size={90} />
    <div style={{ fontSize: 9, color: "#9ca3af", marginTop: 4 }}>{enemy.name}</div>
  </div>
</div>

{won && (
  <div style={{ background: "rgba(74,222,128,0.1)", border: "1px solid #4ade8044", borderRadius: 10, padding: "12px 24px", marginBottom: 20, textAlign: "center", animation: "fadeInUp 0.5s ease 0.3s both" }}>
    <div style={{ fontSize: 11, color: "#4ade80", marginBottom: 6 }}>★ 報酬獲得！</div>
    <div style={{ fontSize: 24, color: "#f59e0b" }}>🪙 +150コイン</div>
  </div>
)}

<button onClick={onHome} style={{ padding: "16px 48px", background: "linear-gradient(135deg,#374151,#1f2937)", border: "2px solid #6b7280", borderRadius: 8, color: "#fff", fontSize: 15, fontWeight: "900", cursor: "pointer", fontFamily: "monospace", letterSpacing: 2, boxShadow: "0 4px 20px rgba(0,0,0,0.5)" }}>
  🏠 ホームへ
</button>
```
...
<div style={{ textAlign: “center” }}>
<button onClick={onBack} style={{ padding: “10px 30px”, background: “transparent”, border: “1px solid #374151”, borderRadius: 8, color: “#6b7280”, fontFamily: “monospace”, cursor: “pointer”, fontSize: 12 }}>← もどる</button>
</div>
</div>
);
};
...
// カットイン表示用
const [cutInCard, setCutInCard]     = useState(null); // {card, cardType}
const [cutInAfter, setCutInAfter]   = useState(null); // cutIn後に実行するscreen名
...
// キャラカード選択完了 → サポートカード選択へ
const handleCharCardSelect = (card) => {
setSelectedCard(card);
setBattleEnemy(ENEMIES[Math.floor(Math.random() * ENEMIES.length)]);
setScreen(“support_select”);
};

// サポートカード選択完了 → イベントカード選択へ
const handleSupportSelect = (card) => {
setSelectedSupport(card);
setScreen(“event_select”);
};

// イベントカード選択完了 → VSへ（サポートカットインはVS後にバトル画面内で）
const handleEventSelect = (card) => {
...
return (
<>
<style>{`@keyframes titlePulse { 0%,100%{text-shadow:0 0 40px #f59e0b, 0 0 80px #f59e0b44} 50%{text-shadow:0 0 60px #f59e0b, 0 0 120px #f59e0b66, 0 0 160px #f59e0b22} } @keyframes rayRotate { from{transform:rotate(0deg)} to{transform:rotate(360deg)} } @keyframes cardAppear { 0%{opacity:0;transform:scale(0.3) translateY(60px) rotateZ(-10deg)} 60%{opacity:1;transform:scale(1.05) translateY(-5px) rotateZ(2deg)} 100%{opacity:1;transform:scale(1) translateY(0) rotateZ(0)} } @keyframes scanline { 0%{top:-10%} 100%{top:110%} } @keyframes dataFloat { 0%{opacity:0.3;transform:translateY(0)} 100%{opacity:0.8;transform:translateY(-8px)} } @keyframes btnGlow { from{box-shadow:0 4px 20px #00ff4488, 0 0 30px #00ff4433} to{box-shadow:0 4px 30px #00ff44cc, 0 0 50px #00ff4466} } @keyframes confettiFall { 0%{transform:translateY(-20px) rotate(0deg);opacity:1} 100%{transform:translateY(100vh) rotate(720deg);opacity:0} } @keyframes fadeInUp { from{opacity:0;transform:translateY(20px)} to{opacity:1;transform:translateY(0)} } @keyframes scaleIn { from{opacity:0;transform:scale(0.5)} to{opacity:1;transform:scale(1)} } @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.5} } @keyframes damageFloat { 0%{opacity:1;transform:translateY(0) scale(1)} 100%{opacity:0;transform:translateY(-40px) scale(1.3)} } @keyframes winBounce { from{transform:translateY(0) rotate(-5deg)} to{transform:translateY(-10px) rotate(5deg)} } @keyframes fightIn { 0%{opacity:0;transform:scale(2.5) rotate(-5deg)} 60%{opacity:1;transform:scale(0.95)} 100%{opacity:1;transform:scale(1)} } @keyframes fightOut { 0%{opacity:1;transform:scale(1)} 100%{opacity:0;transform:scale(0.5)} } @keyframes setIn { 0%{opacity:0;transform:scale(0.4) rotate(-8deg)} 60%{opacity:1;transform:scale(1.1) rotate(2deg)} 100%{opacity:1;transform:scale(1)} } @keyframes kiryokuIn { 0%{opacity:0;transform:scale(0.3) rotate(-6deg)} 60%{opacity:1;transform:scale(1.05)} 100%{opacity:1;transform:scale(1)} } @keyframes atkUpIn { 0%{opacity:0;transform:scale(0) rotate(-12deg)} 60%{opacity:1;transform:scale(1.1)} 100%{opacity:1;transform:scale(1)} } @keyframes goldShine { 0%,100%{filter:brightness(1)} 50%{filter:brightness(1.35) drop-shadow(0 0 8px #fbbf24)} } @keyframes starFloat { 0%{opacity:1;transform:translateY(0) scale(1)} 100%{opacity:0;transform:translateY(-30px) scale(0.5)} } @keyframes dmgTextIn { 0%{opacity:0;transform:scale(0.4) translateY(10px)} 60%{opacity:1;transform:scale(1.05) translateY(-5px)} 100%{opacity:1;transform:scale(1) translateY(0)} } @keyframes dragonBurstIn { 0%{opacity:0;transform:scale(0.2) rotate(-8deg)} 50%{opacity:1;transform:scale(1.08) rotate(1deg)} 100%{opacity:1;transform:scale(1)} } @keyframes clashFlashAnim { 0%{opacity:1} 100%{opacity:0} } @keyframes coreFlash { 0%{transform:translate(-50%,-50%) scale(0.7);opacity:0.8} 100%{transform:translate(-50%,-50%) scale(1.2);opacity:1} } @keyframes sparkRay { 0%{transform:translate(0,-50%) rotate(var(--deg,0deg)) scaleX(0.6);opacity:0.6} 100%{transform:translate(0,-50%) rotate(var(--deg,0deg)) scaleX(1.2);opacity:1} } @keyframes particle0 { 0%{transform:translate(-50%,-50%) rotate(0deg) translateX(8px) scale(1);opacity:1} 100%{transform:translate(-50%,-50%) rotate(360deg) translateX(30px) scale(0);opacity:0} } @keyframes particle1 { 0%{transform:translate(-50%,-50%) rotate(0deg) translateX(6px) scale(1);opacity:1} 100%{transform:translate(-50%,-50%) rotate(-360deg) translateX(26px) scale(0);opacity:0} } @keyframes particle2 { 0%{transform:translate(-50%,-50%) rotate(0deg) translateX(10px) scale(1);opacity:1} 100%{transform:translate(-50%,-50%) rotate(270deg) translateX(34px) scale(0);opacity:0} } @keyframes vsTextIn { 0%{opacity:0;transform:translate(-50%,-50%) scale(3) rotate(-4deg)} 60%{opacity:1;transform:translate(-50%,-50%) scale(0.95)} 100%{opacity:1;transform:translate(-50%,-50%) scale(1)} } @keyframes vsCharIn { 0%{opacity:0;transform:translateX(-60px) scale(0.7)} 60%{opacity:1;transform:translateX(5px) scale(1.02)} 100%{opacity:1;transform:translateX(0) scale(1)} } @keyframes vsEnemyIn { 0%{opacity:0;transform:translateX(60px) scale(0.7)} 60%{opacity:1;transform:translateX(-5px) scale(1.02)} 100%{opacity:1;transform:translateX(0) scale(1)} } @keyframes btnPulse { 0%,100%{transform:scale(1)} 50%{transform:scale(1.05)} } @keyframes rouletteCountPop { 0%{opacity:0;transform:scale(2.2)} 40%{opacity:0.92;transform:scale(0.92)} 70%{transform:scale(1.06)} 100%{opacity:0.85;transform:scale(1)} } @keyframes cutInTitleSlide { 0%{opacity:0;transform:translateY(-60px)} 100%{opacity:1;transform:translateY(0)} } @keyframes cutInCardIn { 0%{opacity:0;transform:scale(0.2) rotate(-8deg)} 60%{opacity:1;transform:scale(1.06) rotate(1deg)} 100%{opacity:1;transform:scale(1) rotate(0)} } @keyframes cardShine { 0%{left:-60%} 100%{left:120%} } @keyframes cutInBannerIn { 0%{opacity:0;transform:scaleX(0.1) scaleY(1.5)} 50%{opacity:1;transform:scaleX(1.05) scaleY(0.95)} 100%{opacity:1;transform:scale(1)} } @keyframes fusionCharLeft { 0%{transform:translateX(0) scale(1)} 70%{transform:translateX(40px) scale(1.2)} 90%{transform:translateX(30px) scale(0.8)} 100%{transform:translateX(0) scale(0);opacity:0} } @keyframes fusionCharRight { 0%{transform:translateX(0) scale(1)} 70%{transform:translateX(-40px) scale(1.2)} 90%{transform:translateX(-30px) scale(0.8)} 100%{transform:translateX(0) scale(0);opacity:0} } @keyframes kiEnter { 0%{transform:translateX(-50%) translateY(-280px) scale(0.6);opacity:0} 20%{opacity:1;transform:translateX(-50%) translateY(-200px) scale(1)} 80%{opacity:1;transform:translateX(-50%) translateY(-10px) scale(1.1)} 100%{transform:translateX(-50%) translateY(0px) scale(1.3);opacity:0.8} } @keyframes charLand { 0%{transform:scaleY(0.6) scaleX(1.3);opacity:0.5} 40%{transform:scaleY(1.1) scaleX(0.95)} 70%{transform:scaleY(0.97) scaleX(1.02)} 100%{transform:scale(1);opacity:1} } @keyframes scouterZoomIn { 0%{opacity:0;transform:scale(1.4)} 60%{opacity:1;transform:scale(2.1)} 100%{opacity:1;transform:scale(2.2)} } @keyframes scouterStatIn { 0%{opacity:0;transform:translateX(14px)} 100%{opacity:1;transform:translateX(0)} } @keyframes scouterScan { 0%{top:-4px} 100%{top:100%} } @keyframes supportBannerSlide { 0%{opacity:0;transform:translateX(-60%) skewX(-8deg)} 60%{opacity:1;transform:translateX(4%) skewX(-8deg)} 100%{opacity:1;transform:translateX(0%) skewX(-8deg)} } @keyframes eventCutIn { 0%{opacity:0;transform:scale(0.3) translateY(40px)} 60%{opacity:1;transform:scale(1.05) translateY(-4px)} 100%{opacity:1;transform:scale(1) translateY(0)} } @keyframes stickman-idle { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-4px)} } @keyframes stickman-attack { 0%{transform:translateX(0)} 30%{transform:translateX(20px)} 100%{transform:translateX(0)} } @keyframes stickman-hit { 0%,100%{transform:translateX(0)} 25%{transform:translateX(-12px)} 75%{transform:translateX(4px)} } @keyframes stickman-win { from{transform:translateY(0) rotate(-5deg)} to{transform:translateY(-10px) rotate(5deg)} } @keyframes stickman-lose { to{transform:translateY(8px) rotate(15deg);opacity:0.5} } .stickman-idle { animation: stickman-idle 2s ease-in-out infinite; } .stickman-attack { animation: stickman-attack 0.4s ease; } .stickman-hit { animation: stickman-hit 0.3s ease; } .stickman-win { animation: stickman-win 0.6s ease infinite alternate; } .stickman-lose { animation: stickman-lose 0.5s ease forwards; } * { box-sizing:border-box; margin:0; padding:0; } body { background:#000; } button:active { transform:scale(0.97); } img { -webkit-user-drag:none; user-select:none; }`}</style>
...
```
  ...
  {screen === "gacha_result" && gachaCard && (
    <GachaResultScreen
      card={gachaCard} mode={gachaMode}
      onHome={() => setScreen("title")}
      onBuyAgain={handleBuyAgain}
      onSelectCard={() => setScreen("select")}
    />
  )}
  ...
  {screen === "select" && (
    <CardSelectScreen
      ...
      onSelect={handleCharCardSelect}
      onBack={() => setScreen("title")}
    />
  )}

  {/* ② サポートカード選択 */}
  {screen === "support_select" && (
    <SupportCardSelectScreen
      ownedSupports={ownedSupports}
      onSelect={handleSupportSelect}
      onSkip={handleSupportSelect}
    />
  )}

  {/* ③ イベントカード選択 */}
  {screen === "event_select" && (
    <EventCardSelectScreen
      ownedEvents={ownedEvents}
      onSelect={handleEventSelect}
      onSkip={handleEventSelect}
    />
  )}

  {/* カードカットイン演出 */}
  {screen === "card_cutin" && cutInCard && (
    <CardCutIn
      card={cutInCard.card}
      cardType={cutInCard.cardType}
      onDone={() => {
        setCutInCard(null);
        setScreen(cutInAfter || "vs");
      }}
    />
  )}

  {/* ④ VS カットシーン */}
  {screen === "vs" && selectedCard && battleEnemy && (
    <VSCutScreen
      playerCard={selectedCard}
      enemyData={battleEnemy}
      onDone={() => setScreen("battle")}
    />
  )}

  {/* ⑤ バトル */}
  {screen === "battle" && selectedCard && battleEnemy && (
    <BattleScreen
      playerCard={selectedCard}
      enemyData={battleEnemy}
      supportCard={selectedSupport}
      eventCard={selectedEvent}
      onEnd={won => {
        if (won) setCoins(c => c + 150);
        setBattleResult(won);
        setTimeout(() => setScreen("result"), 500);
      }}
      onEventCutIn={(card, cardType, afterFn) => {
        setCutInCard({ card, cardType });
        setCutInAfter("battle");
        // cutIn後にafterFnをコールバックで渡す手段としてlocalStorageは使えないため
        // BattleScreen内でイベントカットインは完結させる（screen遷移なし）
      }}
    />
  )}

  {/* ⑥ リザルト */}
  {screen === "result" && selectedCard && battleEnemy && (
    <ResultScreen won={battleResult} playerCard={selectedCard} enemy={battleEnemy}
      onHome={() => {
        setBattleResult(null); setSelectedCard(null);
        setSelectedSupport(null); setSelectedEvent(null);
        setScreen("title");
      }}
    />
  )}
